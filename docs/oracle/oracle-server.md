...
- `createnumericevent` `name` `maturationtime` `maxvalue` `unit` `precision` `[signed]` - Registers an oracle event that uses digit decomposition when signing the number
...
CURL:
```bash
$ curl --data-binary '{"jsonrpc": "1.0", "id": "curltest", "method": "createenumevent", "params": ["testEvent", 1701917137, ["outcome1", "outcome2", "outcome3"]]}' -H "Content-Type: application/json" http://127.0.0.1:9998/
...
```bash
$ bitcoin-s-cli createnumericevent exampleNumeric 1701917137 10 3 "units" 0 --signed
...
$ bs-cli getevent exampleNumeric
{
  "nonces": [
...
```bash
$ curl --data-binary '{"jsonrpc": "1.0", "id": "curltest", "method": "createnumericevent", "params": ["numericExample", 1701917137, 100, true, "units", 0]}' -H "Content-Type: application/json" http://127.0.0.1:9998/
