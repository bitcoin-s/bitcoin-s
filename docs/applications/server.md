...
### DLC
 ...
 - `getdlc` `dlcId` - Gets a specific dlc in the wallet
    - `dlcId` - Internal id of the DLC
 - `registerincomingoffer` `offerTLV` `peer` `message` - Puts an incoming offer into the inbox
    - `offerTLV` - Offer TLV
    - `peer` - Peer URI (optional)
    - `message` - Peer's message or note (optional)
 - `rejectincomingoffer` `hash` - remove an incoming offer from inbox
    - `hash` - Hash of the offer TLV
 - `listincomingoffers` - List all incoming offers from the inbox
