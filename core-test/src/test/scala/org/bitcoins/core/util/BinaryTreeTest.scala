...
class BinaryTreeTest extends BitcoinSUnitTest {
  ...
  it must "convert a binary tree to to a list with node values" in {
    ...
    val bTree: BinaryTree[ScriptToken] =
      Node[ScriptToken](
        OP_IF,
        Node(OP_IF, Leaf(OP_1), Node(OP_ELSE, Leaf(OP_0), Leaf(OP_ENDIF))),
        Node(OP_ELSE,
  ...
  it must "convert a simple binary tree to a sequence" in {
    ...
    val bTree2 = Node(OP_IF,
                      Node(OP_1, Empty, Empty),
                      Node(OP_ELSE, Node(OP_2, Empty, Empty), Leaf(OP_ENDIF)))
  ...
  it must "find the first occurrence of a element in the tree" in {
    val tree =
      Node[Int](0, Node(1, Leaf(2), Leaf(3)), Node(1, Leaf(4), Leaf(5)))
  ...
  it must "tell if a tree contains a certain element" in {
    val tree = Node[String]("Hello",
                            Node("there", Leaf("1"), Leaf("2")),
                            Node("3", Empty, Leaf("4")))
  ...
  it must "remove a subtree" in {
    ...
    tree.remove(subTree)() must be(
      Node("Hello", Empty, Node("3", Empty, Leaf("4"))))
  }
  ...
  it must "replace the left branch of a tree" in {
    val tree = Node[String]("Hello",
                            Node("there", Leaf("1"), Leaf("2")),
                            Node("3", Empty, Leaf("4")))
    tree.replace(tree.left.get, tree.right.get)() must be(
      Node[String]("Hello",
                   Node("3", Empty, Leaf("4")),
                   Node("3", Empty, Leaf("4"))))
  }
  ...
  it must "replace the right branch of a tree" in {
    val tree = Node[String]("Hello",
                            Node("there", Leaf("1"), Leaf("2")),
                            Node("3", Empty, Leaf("4")))
    tree.replace(tree.right.get, tree.left.get)() must be(
      Node[String]("Hello",
                   Node("there", Leaf("1"), Leaf("2")),
                   Node("there", Leaf("1"), Leaf("2"))))
  }
