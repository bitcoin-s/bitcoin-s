...
class ScriptInterpreterTest extends BitcoinSUnitTest {
  ...
  /**
    * Converts a test case into a script program
    * that can be fed into our script interpreter
    */
  private def getProgram(testCase: CoreTestCase): PreExecutionScriptProgram = {

    val (creditingTx, outputIndex) = TransactionTestUtil
      .buildCreditingTransaction(testCase.scriptPubKey,
                                 testCase.witness.map(_._2))

    val (tx, inputIndex) = TransactionTestUtil.buildSpendingTransaction(
      creditingTx,
      testCase.scriptSig,
      outputIndex,
      testCase.witness)
  ...
  behavior of "ScriptInterpreter"

  it must "evaluate all the scripts from the bitcoin core script_tests.json" in {
   ...
   """.stripMargin*/
    for (testCase <- testCases) yield {
      val program = getProgram(testCase)

      withClue(testCase.raw) {
        val runAttemptT = Try(ScriptInterpreter.run(program))
        runAttemptT match {
          case Failure(exception) => fail(exception)
          case Success(result)    => assert(result == testCase.expectedResult)
        }
