...
class ControlOperationsInterpreterTest extends BitcoinSJvmTest {
  ...
  it must "evaluate an OP_IF block correctly if the stack top is true" in {
    ...
    val elseProgram =
      newProgramInProgress.updateStackAndScript(
        List(ScriptNumber.one),
