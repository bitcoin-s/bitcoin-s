...
abstract class MerkleGenerator {
  ...
  /** Generates a merkle block with the given txs matched inside the
    ...
    */
  def merkleBlockWithInsertedTxIds(txs: Seq[Transaction])
      : Gen[(MerkleBlock, Block, Seq[DoubleSha256Digest])] =
    for {
  ...
  /** Returns a
    ...
    * including the sequence of hashes inserted in to the bloom filter
    */
  ...
  /** Returns a
    * [[org.bitcoins.core.protocol.blockchain.MerkleBlock MerkleBlock]] created
    * with a [[org.bitcoins.core.bloom.BloomFilter BloomFilter]], with the block
    * it was created from and the transactions that were matched inside of that
    * block NOTE: Since bloom filters can produce false positives, it is
    * possible that there will be matches in the parital merkle tree that SHOULD
    * NOT be matched. Bloom filters do not guaratnee no false negatives.
    * @return
    */
  def merkleBlockCreatedWithBloomFilter
      : Gen[(MerkleBlock,
             Block,
  ...
  /** Generates a partial merkle tree with a sequence of txids and a flag
  ...
  /** Generates a transaction ids with a boolean indicator if they match the
    * bloom filter or not this is useful for testing partial merkle trees as
  ...
  /** Generates a list of txids with a boolean indicator signifying if it
    * matched the bloom filter or not
    */
