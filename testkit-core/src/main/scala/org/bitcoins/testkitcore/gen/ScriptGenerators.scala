...
sealed abstract class ScriptGenerators {
  ...
  private[gen] def redeemScriptTooBig(redeemScript: ScriptPubKey): Boolean = {
    redeemScript.compactSizeUInt.toInt + CompactSizeUInt(
      UInt64(ScriptInterpreter.MAX_PUSH_SIZE)
    ).bytes.length >= ScriptInterpreter.MAX_PUSH_SIZE
  }
  ...
  def conditionalScriptPubKey(
      maxDepth: Int
  ): Gen[(ConditionalScriptPubKey, Seq[ECPrivateKey])] = {
  ...
  def signedCLTVScriptSignature(
    ...
    for {
      ...
      case m: MultiSignatureScriptPubKey =>
        ...
        val cltvScriptSig = lockTimeHelper(
          Some(lockTime),
          sequence,
          cltv,
          privKeys,
  ...
  def signedCSVScriptSignature(
    ...
    for {
      ...
      case m: MultiSignatureScriptPubKey =>
        val requiredSigs = m.requiredSigs
        val csvScriptSig = lockTimeHelper(
          None,
          sequence,
          csv,
          privKeys,
