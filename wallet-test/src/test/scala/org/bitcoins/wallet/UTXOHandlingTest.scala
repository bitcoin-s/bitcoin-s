...
class UTXOHandlingTest extends BitcoinSUnitTest {

  behavior of "UTXOHandling"

  val utxo = sampleSegwitUTXO(
    EmptyScriptPubKey,
  ...
  it must "correctly update spent txo state based on confirmations" in {
    ...
    assertThrows[RuntimeException] {
      ...
      UtxoHandling.updateSpentTxoWithConfs(immatureCoinbase, 100, requiredConfs)
    }
    ...
    assert(
      UtxoHandling.updateSpentTxoWithConfs(
        ...
        1,
        requiredConfs
      ) == confSpent.copyWithState(PendingConfirmationsSpent)
    )
