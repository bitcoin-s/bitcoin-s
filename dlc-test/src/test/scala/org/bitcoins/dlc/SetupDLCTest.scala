...
import org.bitcoins.core.crypto.{DoubleSha256DigestBE, ECPublicKey}
...
class SetupDLCTest extends BitcoinSAsyncTest {
  ...
  val validSetupDLC: SetupDLC = SetupDLC(
    fundingTx = validFundingTX,
    cetWin = validCET,
    cetWinWitness = dummyWitness,
    cetLose = validCET,
    cetLoseWitness = dummyWitness,
    cetWinRemoteTxid = dummyTxId,
    cetWinRemoteWitness = dummyWitness,
    cetLoseRemoteTxid = dummyTxId,
    cetLoseRemoteWitness = dummyWitness,
    refundTx = validRefundTx
  )

  it must "not allow an invalid number of inputs for CETs" in {
    // Funding tx has more than 1 input
    assertThrows[IllegalArgumentException](
      validSetupDLC.copy(cetWin = validFundingTX))
    assertThrows[IllegalArgumentException](
      validSetupDLC.copy(cetLose = validFundingTX))
  }

  it must "not allow an invalid input for CETs" in {
    assertThrows[IllegalArgumentException](
      validSetupDLC.copy(cetWin = invalidCET))
    assertThrows[IllegalArgumentException](
      validSetupDLC.copy(cetLose = invalidCET))
  }

  it must "not allow an invalid number of inputs for the refundTx" in {
    // Funding tx has more than 1 input
    assertThrows[IllegalArgumentException](
      validSetupDLC.copy(refundTx = validFundingTX))
  }

  it must "not allow an invalid input for the refundTx" in {
    assertThrows[IllegalArgumentException](
      validSetupDLC.copy(refundTx = invalidRefundTx))
  }
}
