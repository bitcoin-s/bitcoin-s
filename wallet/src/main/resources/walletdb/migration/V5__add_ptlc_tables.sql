...
CREATE TABLE "wallet_ptlc_invoices" ("id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,"invoiceId" VARCHAR(254) NOT NULL UNIQUE,"adaptorScalar" VARCHAR(254) NOT NULL,"adaptorPoint" VARCHAR(254) NOT NULL,"amount" INTEGER NOT NULL,"pubkey" VARCHAR(254) NOT NULL,"finalAddress" VARCHAR(254) NOT NULL,"timeout" INTEGER NOT NULL,constraint "fk_invoiceId" foreign key("invoiceId") references "wallet_ptlcs"("invoiceId") on update NO ACTION on delete NO ACTION);
