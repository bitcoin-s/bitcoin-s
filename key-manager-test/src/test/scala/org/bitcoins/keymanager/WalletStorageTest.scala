...
class WalletStorageTest extends BitcoinSWalletTest with BeforeAndAfterEach {
  ...
  it must "read an encrypted mnemonic without a creation time" in {
    ...
    """.stripMargin
      ...
      val read =
        WalletStorage.decryptSeedFromDisk(
          seedPath,
  ...
  it must "write and read an encrypted ExtPrivateKey from disk" in {
    walletConf =>
      ...
      val expected = BIP39Seed
        .fromMnemonic(
          mnemonic = writtenMnemonic.mnemonicCode,
          password = password
        )
  ...
  it must "backup an unencrypted seed" in { walletConf =>
    ...
    val words = WalletStorage
      .readDecryptedSeedPhraseForBackup(walletConf.seedPath, None)
      .get
    assert(words == mnemonicCode.words)

    WalletStorage.markSeedAsBackedUp(walletConf.seedPath, None)

    WalletStorage.decryptSeedFromDisk(walletConf.seedPath, None) match {
      case Right(decryptedMnemonic) =>
        assert(decryptedMnemonic.backupTimeOpt.nonEmpty)
      case Left(err) => fail(err.toString)
    }

    assertThrows[AlreadyBackedUpException](
      WalletStorage
        .readDecryptedSeedPhraseForBackup(walletConf.seedPath, None)
        .get
    )
  ...
  it must "backup an encrypted seed" in { walletConf =>
    ...
    val words = WalletStorage
      .readDecryptedSeedPhraseForBackup(walletConf.seedPath, passphrase)
      .get
    assert(words == mnemonicCode.words)

    WalletStorage.markSeedAsBackedUp(walletConf.seedPath, passphrase)
