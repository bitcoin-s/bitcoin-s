...
import org.bitcoins.core.protocol.{Bech32Address, BlockStamp, BlockTimeStamp}
...
object DLCTestVector {
  ...
  def fromInputs(
      ...
      remoteChangeSPK: WitnessScriptPubKeyV0,
      timeouts: DLCTimeouts,
      feeRate: SatoshisPerVirtualByte)(implicit
      ec: ExecutionContext): Future[DLCTestVector] = {
    ...
    val acceptDLC = TestDLCClient(
      ...
      winPayout =
        localInput + remoteInput - localPayouts(possibleOutcomes.head),
      losePayout =
        localInput + remoteInput - localPayouts(possibleOutcomes.last),
      timeouts = timeouts,
      feeRate = feeRate,
    ...
    for {
      ...
      unilateralOutcome <-
        offerDLC.executeUnilateralDLC(offerSetup, Future.successful(oracleSig))
      ...
      _ <- offerDLC.initiateMutualClose(offerSetup,
                                        oracleSig,
                                        { (sig, offerSig) =>
...
object SerializedDLCTestVectorSerializers {
  ...
  implicit val blockStampWithFutureWrites: Writes[BlockTimeStamp] =
    Writes[BlockTimeStamp] { blockStamp =>
  ...
  def hexReads[T <: NetworkElement](factory: Factory[T]): Reads[T] =
    Reads[T] { json =>
  ...
  implicit val blockStampWithFutureReads: Reads[BlockTimeStamp] =
    Reads[BlockTimeStamp] { json =>
  ...
  implicit val scriptPubKeyReads: Reads[WitnessScriptPubKeyV0] =
    Reads[WitnessScriptPubKeyV0] { json =>
      json
        ...
        .map(_.scriptPubKey.asInstanceOf[WitnessScriptPubKeyV0])
    }
