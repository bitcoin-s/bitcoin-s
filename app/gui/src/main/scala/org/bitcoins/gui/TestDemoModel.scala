...
import org.bitcoins.core.crypto.{ECPrivateKey, Schnorr, SchnorrNonce}
import org.bitcoins.dlc.DLCMessage.OracleInfo
import org.bitcoins.gui.dialog.{
  AcceptCloseDLCDialog,
  AcceptDLCDialog,
  AddSigsDLCDialog,
  DLCDialog,
  ForceCloseDLCDialog,
    ...
    result match {
      case Some((outcomes, contractInfo)) =>
        ...
        outcomes.zip(contractInfo.keys).foreach {
          case (outcome, hash) =>
            val sig = Schnorr.signWithNonce(hash.bytes, privKey, nonce)
            builder.append(s"$outcome - ${sig.hex}\n")
        }

        GlobalData.lastOracleInfo = oracleInfo.hex
        GlobalData.lastContractInfo = contractInfo.hex
