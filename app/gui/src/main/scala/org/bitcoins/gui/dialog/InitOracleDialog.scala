...
object InitOracleDialog {
  ...
  def showAndWait(
    ...
    val fields =
      (0 until numOutcomes).map(_ => (new TextField(), new TextField()))
    ...
    result match {
      case Some(Some((outcomes: Vector[String], contractInfo: ContractInfo))) =>
        Some((outcomes, contractInfo))
