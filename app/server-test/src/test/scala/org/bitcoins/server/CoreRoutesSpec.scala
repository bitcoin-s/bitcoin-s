...
class CoreRoutesSpec
  ...
  private val expectedAcceptJsonString: String =
    s"""
       |{
       |  "result":
       |    {
       ...
       |}
       |""".stripMargin
  private val expectedAccept = ujson.read(expectedAcceptJsonString)
  ...
  private val expectedSignJsonString: String = s"""
                                                  |{
                                                  |  "result":
                                                  |
                                                  |{
  ...
  private val expectedSign = ujson.read(expectedSignJsonString)
  "Core routes" should {
    "decode an accept message" in {
      val args = ujson.Arr(acceptHex)
      val route =
      ...
      Post() ~> route ~> check {
        ...
        assert(actualJson == expectedAccept)
      }
    }

    "decode a sign message" in {
      val args = ujson.Arr(signHex)
      val route =
      ...
      Post() ~> route ~> check {
        assert(contentType == ContentTypes.`application/json`)
        val actualJson = ujson.read(responseAs[String])
        assert(actualJson == expectedSign)
      }
    }
  }
