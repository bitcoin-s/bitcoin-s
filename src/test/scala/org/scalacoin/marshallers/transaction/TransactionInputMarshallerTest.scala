...
import org.scalacoin.script.constant.{BytesToPushOntoStackImpl, OP_0, ScriptConstantImpl}
...
class TransactionInputMarshallerTest extends FlatSpec with MustMatchers {
  ...
  "TransactionInputMarshaller" must "marshall a json input" in {
    val expectedScript = List(OP_0, BytesToPushOntoStackImpl(72),
