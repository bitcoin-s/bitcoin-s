...
import org.scalacoin.protocol.script.{ScriptSignature, ScriptSignatureFactory, ScriptPubKey, ScriptPubKeyFactory}
import org.scalacoin.protocol.transaction.{TransactionFactory, Transaction}
...
object TestUtil {
  ...
  def p2pkhScriptSig = ScriptSignatureFactory.fromHex(p2pkhInputScript)
  ...
  val p2shInputScript = ScriptSignatureFactory.fromHex(rawP2shInputScript)
  ...
  def p2shInputScriptSigHashSingle = ScriptSignatureFactory.fromHex(rawP2shInputScriptSigHashSingle)
  ...
  def p2shInputScript2Of2 = ScriptSignatureFactory.fromHex(rawP2shInputScript2Of2)
  def p2sh2Of2Tx = TransactionFactory.fromHex(rawP2SH2Of2Tx)
  ...
  def p2sh2Of2CreditingTx = TransactionFactory.fromHex(rawP2SH2Of2CreditingTx)
  ...
  def p2shInputScriptLargeSignature = ScriptSignatureFactory.fromHex(rawP2shInputScriptLargeSignature)
  ...
  def p2sh2Of3ScriptSig = ScriptSignatureFactory.fromHex(rawP2sh2Of3ScriptSig)
  ...
  def transaction = TransactionFactory.fromHex(rawTransaction)
  ...
  def simpleTransaction = TransactionFactory.fromHex(simpleRawTransaction)
  ...
  def parentSimpleTransaction = TransactionFactory.fromHex(parentSimpleRawTransaction)
  ...
  def scriptSigNotStrictDerEncoded = ScriptSignatureFactory.fromHex(rawScriptSigNotStrictDerEncoded)

  val p2pkhScriptSigNotStrictDerEncoded  = ScriptSignatureFactory.fromAsm(List(BytesToPushOntoStackImpl(71),
