...
class TransactionInputFactoryTest extends FlatSpec with MustMatchers {
  ...
  "TransactionInputFactory" must "create a transaction input out of it's base components" in {
    ...
    val input = TransactionInputFactory.factory(EmptyTransactionInput.previousOutput,EmptyTransactionInput.scriptSignature, EmptyTransactionInput.sequence)
  ...
  it must "modify an input from an output and that output's transaction" in {
    ...
    val newInput = TransactionInputFactory.factory(input,TestUtil.simpleTransaction.outputs(0), TestUtil.simpleTransaction)
  ...
  it must "chage the input's outpoint to the given outpoint" in {
    val input = TestUtil.txInput.head
    val newInput = TransactionInputFactory.factory(input,EmptyTransactionOutPoint)
