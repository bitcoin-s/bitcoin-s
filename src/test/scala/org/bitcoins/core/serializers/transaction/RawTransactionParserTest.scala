...
class RawTransactionParserTest extends FlatSpec with MustMatchers {
  ...
  "RawTransactionParser" must "parse a raw transaction" in {
    ...
    tx.txId.hex must be ("44e504f5b7649d215be05ad9f09026dee95201244a3b218013c504a6a49a26ff")
  }
  it must "parse a transaction correctly with a locktime" in {
    ...
    tx.txId.hex must be ("bdc221db675c06dbee2ae75d33e31cad4e2555efea10c337ff32c8cdf97f8e74")
  ...
  it must "parse a transaction with one input and two outputs" in {
    ...
    tx.inputs.head.previousOutput.txId.hex must be ("65bd23d3fb0ac9d3ee0195aae8d033d6689dacf71907902b27a7ad6f6441a7cd")
