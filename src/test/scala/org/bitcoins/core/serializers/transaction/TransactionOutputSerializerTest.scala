 ...
 */
class TransactionOutputSerializerTest extends FlatSpec with MustMatchers {
  val str =
    """
      |{
      |   "value" : 0.00020000,
  ...
  "TranasctionOutput" must "parse an output from json" in {
    val output : TransactionOutput = TransactionOutputSerializer.TransactionOutputFormatter.read(json)
  ...
  it must "write a transaction output" in {
    val output : TransactionOutput = TransactionOutputSerializer.TransactionOutputFormatter.read(json)
    val writtenOutput = TransactionOutputSerializer.TransactionOutputFormatter.write(output)
