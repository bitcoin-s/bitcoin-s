 ...
 */
class TransactionOutPointSerializerTest extends FlatSpec with MustMatchers {
  ...
  "TransactionOutPointMarshaller" must "parse the transaction outpoint from a json input" in {
    ...
    val outPoint : TransactionOutPoint = TransactionOutPointSerializer.TransactionOutPointFormatter.read(json)
  ...
  it must "write a transaction outpoint" in {
    val json = str.parseJson
    val outPoint : TransactionOutPoint = TransactionOutPointSerializer.TransactionOutPointFormatter.read(json)
    val writtenOutPoint = TransactionOutPointSerializer.TransactionOutPointFormatter.write(outPoint)
