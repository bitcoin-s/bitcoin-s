...
class RawMerkleBlockSerializerTest extends FlatSpec with MustMatchers {
  ...
  it must "not have any extra hashes left over when deserializing a previously valid partial merkle tree" in {
    val (merkleBlock,txIds) = (MerkleBlock(BlockHeader(UInt32(1626792925),
      ...
      PartialMerkleTree(Node(
        ...
        Node(DoubleSha256Digest("2dc060204deff176f6e366319777d4db76546c17c07f23ebeece4bf0e66fd686"),
          Node(DoubleSha256Digest("8023ea73c6c1e643d1604585abbfd997308a5759baa4d2d4a2ee876d71a5780f"),
          ...
          Node(DoubleSha256Digest("79a03d2d9f1c5c97772974c9ef9297e6e2bce0271ca95d40bb598c6156c3d6e0"),
        ...
        List(DoubleSha256Digest("e4aeaf729035a7fb939e12c4f6a2072a9b2e7da784207ce7852d398593210a45"),
      ...
      List(DoubleSha256Digest("010506d2103d0feb477224926eedaf3d7478fe3d93b54bd24e5eb2c0adc309b3"),
        DoubleSha256Digest("7ae10c30932c07e4ed25abab233565f9ab279eabbcd60e1bc028c6cdc400361b")))
