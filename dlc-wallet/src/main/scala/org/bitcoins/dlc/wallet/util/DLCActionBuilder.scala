...
case class DLCActionBuilder(dlcWalletDAOs: DLCWalletDAOs) {
  ...
  def buildCreateOfferAction(
      ...
      dlcOfferDb: DLCOfferDb)(implicit ec: ExecutionContext)
      : DBIOAction[Unit,
                   NoStream,
  ...
  def buildCreateAcceptAction(
      ...
      refundSigsDb: DLCRefundSigsDb)(implicit ec: ExecutionContext)
      : DBIOAction[Unit,
                   NoStream,
  ...
  /** Creates the action to delete the given dlc from our database. This removes
    ...
    */
  def deleteDLCAction(dlcId: Sha256Digest)(implicit ec: ExecutionContext)
  ...
  /** Retrieves a DBIOAction that fetches the global dlc db, the contract, the
    ...
    */
  def getDLCOfferDataAction(dlcId: Sha256Digest)(implicit ec: ExecutionContext)
      : DBIOAction[(Option[DLCDb],
    ...
    val dlcDbOfferDbContractDataDbOptA
        : DBIOAction[Option[((DLCDb, DLCOfferDb), DLCContractDataDb)],
                     NoStream,
                     Effect.Read with Effect.Transactional] = {
      dlcDbQ
        .join(offerDbsQ)
  ...
  def updateDLCOracleSigsAction(
      outcomeAndSigByNonce: Map[SchnorrNonce,
                                (String, SchnorrDigitalSignature)])(implicit
      ec: ExecutionContext)
      : DBIOAction[Vector[OracleNonceDb],
                   NoStream,
                   Effect.Write with Effect.Read with Effect.Transactional] = {
