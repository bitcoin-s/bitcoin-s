...
trait DLCIdDaoUtil[T, PrimaryKeyType] { _: CRUD[T, PrimaryKeyType] =>
  ...
  def findByDLCIdAction(
      dlcId: Sha256Digest): profile.api.DBIOAction[Option[T],
  ...
  def findByDLCIdsAction(dlcId: Vector[Sha256Digest])
      : profile.api.DBIOAction[Vector[T],
                               profile.api.NoStream,
                               profile.api.Effect.Read]
  ...
  def deleteByDLCIdAction(
      dlcId: Sha256Digest): profile.api.DBIOAction[Int,
                                                   profile.api.NoStream,
...
trait DLCIdDaoUtilNoPK[T] { _: CRUD[T, _] =>
  ...
  def findByDLCIdAction(
      dlcId: Sha256Digest): profile.api.DBIOAction[Vector[T],
                                                   profile.api.NoStream,
                                                   profile.api.Effect.Read]
