...
trait DLCWalletCallbacks
  ...
  def executeOnDLCStateChange(
      status: DLCStatus
  )(implicit ec: ExecutionContext): Future[Unit] = {
    onStateChange.execute(
      status,
      (err: Throwable) =>
        logger.error(s"${onStateChange.name} Callback failed with error: ", err)
    )
  }

  def executeOnDLCOfferAdd(
      offerDb: IncomingDLCOfferDb
  )(implicit ec: ExecutionContext): Future[Unit] = {
    onOfferAdd.execute(
      offerDb,
      (err: Throwable) =>
...
object DLCWalletCallbacks extends CallbackFactory[DLCWalletCallbacks] {
  ...
  private case class DLCWalletCallbacksImpl(
    ...
    override def +(other: DLCWalletCallbacks): DLCWalletCallbacks = {
      copy(
        onStateChange = onStateChange ++ other.onStateChange,
  ...
  private val emptyImpl: DLCWalletCallbacksImpl = {
    DLCWalletCallbacksImpl(
      CallbackHandler[DLCStatus, OnDLCStateChange](
        "onDLCStateChange",
        Vector.empty
      ),
