...
class EclairRpcClientTest extends BitcoinSAsyncTest {
  ...
  def executeWithClientOtherClient[T](
      test: (EclairRpcClient, EclairRpcClient) => Future[T]
  ): Future[T] = {
  ...
  it should "send some payments and get the audit info" in {
    for {
      ...
      _ <- EclairRpcTestUtil.awaitUntilPaymentSucceeded(
        client1,
        paymentId,
        duration = 1.second
      )
  ...
  it should "pay an invoice and monitor the payment" in {
    val amt = MilliSatoshis.fromSatoshis(Satoshis(1000))
    val checkPayment = {
      (client: EclairRpcClient, otherClient: EclairRpcClient) =>
        for {
          ...
          assert(paymentResult.amount == amt)
  ...
  it should "check a payment" in {
    ...
    val checkPayment = {
      (client: EclairRpcClient, otherClient: EclairRpcClient) =>
        for {
          _ <- EclairRpcTestUtil.openAndConfirmChannel(client, otherClient)
          invoice <- otherClient.createInvoice("abc", amt)
          info <- otherClient.getInfo
          _ = assert(info.nodeId == invoice.nodeId)
          ...
          assert(sentInfo.head.amount == amt)
        }
    }
          ...
          changeAddrF.flatMap { addr =>
            val amountF =
              bitcoindRpcClientF.flatMap { bitcoindRpcClient =>
                bitcoindRpcClient.getReceivedByAddress(
                  address = addr,
                  minConfirmations = 0
                )
              }
  ...
  it should "fail to authenticate on bad password" in {
    ...
    val badClientF =
      badInstanceF.map(
        new EclairRpcClient(
          _,
          Some(EclairRpcTestClient.sbtBinaryDirectory.toFile)
        )
      )
          ...
          for {
            ...
            _ <-
              EclairRpcTestUtil
                ...
                ](otherClient, invoice.lnTags.paymentHash.hash)
            _ <- EclairRpcTestUtil.awaitUntilPaymentSucceeded(
              client,
              result.parentId
            )
      ...
      (client: EclairRpcClient, otherClient: EclairRpcClient) =>
        {
          for {
            channelId <-
              EclairRpcTestUtil.openAndConfirmChannel(client, otherClient)
            ...
            _ = assert(
              channels.exists(_.state == ChannelState.NORMAL),
  ...
  it should "fail to get received info about an invoice that hasn't been paid to, and then successfully get the info after the payment happened" in {
    val amt = 1000.msat
    for {
      c1 <- clientF
      c2 <- otherClientF
  ...
  it should "monitor an invoice" in {
    ...
    val test = (client: EclairRpcClient, otherClient: EclairRpcClient) => {
      val res = for {
        ...
        received <- otherClient.monitorInvoice(
          invoice,
          interval = 1.seconds,
          maxAttempts = 60
        )
  ...
  it should "update the relay fee of a channel" in {
    val channelAndFeeF = for {
      client1 <- clientF
      client2 <- otherClientF
      channelId <-
        EclairRpcTestUtil.openAndConfirmChannel(client1, client2)
      client <- clientF
