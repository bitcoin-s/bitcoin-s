...
//create a native chain handler for bitcoin-s
  val blockHeaderDAO: BlockHeaderDAO = BlockHeaderDAO(
...
val blockHeaderTableF = {
    ...
    //recreate the table
    val createdTableF = dropTableF.flatMap(_ =>
      ChainDbManagement.createHeaderTable(chainDbConfig))

    createdTableF
  }

val createdGenHeaderF =
    blockHeaderTableF.flatMap(_ => blockHeaderDAO.create(genesisHeader))
...
val chainHandlerF =
    blockchainF.map(blockchain => ChainHandler(blockHeaderDAO, chainAppConfig))
...
val bitcoinsLogF = chainApi101BlocksF.flatMap { chainApi =>
    chainApi.getBlockCount.map(count =>
      logger.info(s"bitcoin-s blockcount=${count}"))
  }
...
val walletF = bitcoinsLogF.flatMap { _ =>
    ...
    val createTablesF =
      dropTablesF.flatMap(_ => WalletDbManagement.createAll(walletDbConfig))
    createTablesF.flatMap { _ =>
      Wallet
        .initialize(walletAppConfig)
...
val transactionOutputIndexF: Future[(Transaction, Int)] = for {
    ...
    val Some((output, index)) = tx.outputs.zipWithIndex.find {
      case (output, index) =>
...
chainApi6BlocksF.onComplete { chainApi =>
    ...
    balanceF.onComplete(balance =>
      logger.info(s"bitcoin-s walllet balance=${balance}"))

    balanceF.flatMap(_ => cleanup())
  }

/** Syncs the give number of blocks to our chain */
  def sync(chainHandlerF: Future[ChainApi], numBlocks: Int)(
    ...
    val chainSyncF = genBlocksF.flatMap { _ =>
      chainHandlerF.flatMap { ch =>
        ChainSync.sync(ch.asInstanceOf[ChainHandler],
                       getBlockHeaderFunc,
