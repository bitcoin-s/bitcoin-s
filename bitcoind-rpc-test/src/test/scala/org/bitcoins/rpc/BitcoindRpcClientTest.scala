...
class BitcoindRpcClientTest
  ...
  class BitcoindRpcClientTest
    ...
    override def beforeAll(): Unit = {
      ...
      BitcoindRpcTestUtil.startServers(
        Vector(walletClient, client, otherClient, pruneClient))
                ...
                client.getTransaction(txid).flatMap { tx =>
                  client.getTxOutProof(Vector(txid)).flatMap { proof =>
                    otherClient.getBalance.flatMap { balanceBefore =>
                      otherClient
                        .importPrivKey(privKey, rescan = false)
                        .flatMap { _ =>
                          otherClient
                            .importPrunedFunds(tx.hex, proof)
                            .flatMap { _ =>
    ...
    it should "be able to invalidate a block" in {
      otherClient.getNewAddress.flatMap { address =>
        BitcoindRpcTestUtil
          .fundMemPoolTransaction(client, address, Bitcoins(3))
          .flatMap { txid =>
              ...
              client2.generate(1).flatMap { blocks2 =>
                client1.getBestBlockHash.flatMap { bestHash1 =>
                  ...
                  client2.getBestBlockHash.flatMap { bestHash2 =>
                    ...
                    val conn =
                      client1.addNode(client2.getDaemon.uri, "onetry")
                ...
                otherClient.getNewAddress.flatMap { address =>
                  client
                    ...
                    .map { transaction =>
                      ...
                      assert(
                        transaction.inputs.head.previousOutput.txId == input0.txId)
                      assert(transaction
                        .inputs(1)
                        .previousOutput
                        .txId == input1.txId)
    ...
    it should "be able to find mem pool ancestors and descendants" in {
      ...
      client.getNewAddress.flatMap { address =>
        BitcoindRpcTestUtil
          .fundMemPoolTransaction(client, address, Bitcoins(2))
          .flatMap { txid1 =>
            client.getRawMemPool.flatMap { mempool =>
        ...
        BitcoindRpcTestUtil
          ...
          .flatMap { txid =>
            client.getMemPoolEntry(txid).flatMap { entry =>
              ...
              client
                .prioritiseTransaction(txid, Bitcoins(1).satoshis)
                .flatMap { tt =>
                  assert(tt)
    ...
    it should "be able to add a multi sig address to the wallet" in {
      ...
      client.getNewAddress(addressType = AddressType.Legacy).flatMap {
        address =>
          client
            .addMultiSigAddress(
              2,
    ...
    it should "be able to change the wallet password" in {
      val newPass = "new_password"
      walletClient.walletLock().flatMap { _ =>
        walletClient.walletPassphraseChange(password, newPass).flatMap { _ =>
          password = newPass
          walletClient.walletPassphrase(password, 1000).flatMap { _ =>
            walletClient.getWalletInfo.flatMap { info =>
              assert(info.unlocked_until.nonEmpty)
                  ...
                  otherClient
                    .addMultiSigAddress(2,
                                        Vector(Left(address1Info.pubkey.get),
                                               Left(address2Info.pubkey.get)))
                    .flatMap { _ =>
                      client.validateAddress(multisig.address).flatMap { _ =>
                        BitcoindRpcTestUtil
                          .fundBlockChainTransaction(client,
                                                     multisig.address,
                                                     Bitcoins(1.2))
            ...
            client
              .createRawTransaction(Vector(input),
                                    Map(address -> Bitcoins(0.5),
                                        changeAddress -> Bitcoins(
                                          output.amount.toBigDecimal - 0.55)))
              .flatMap { ctx =>
    ...
    it should "be able to add and remove a node" in {
      otherClient.addNode(walletClient.getDaemon.uri, "add").flatMap { _ =>
        ...
        otherClient.getAddedNodeInfo(walletClient.getDaemon.uri).flatMap {
          info =>
            assert(info.length == 1)
            ...
            otherClient
              .addNode(walletClient.getDaemon.uri, "remove")
              .flatMap { _ =>
      ...
      otherClient.addNode(walletClient.getDaemon.uri, "add").flatMap { _ =>
        ...
        otherClient.getAddedNodeInfo(walletClient.getDaemon.uri).flatMap {
          info =>
            ...
            otherClient.disconnectNode(walletClient.getDaemon.uri).flatMap {
              _ =>
