...
class FrostTestVectors extends BitcoinSCryptoTest {
  ...
  it must "pass sign_verify_vectors.json" in {
    ...
    vecs.sign_error_test_cases.foreach { err =>
      assertThrows[Exception] {
        ...
        val pubshares =
          err.pubshare_indices.map(vecs.pubshares(_)).map(ECPublicKey.fromBytes)
        val aggNonce =
          FrostNoncePub.fromBytes(vecs.aggnonces(err.aggnonce_index))
        val secnonceOpt =
          err.secnonce_index.flatMap(idx => vecs.secnonces_p0.lift(idx))
        val signingContext = FrostSigningContext(
          n = vecs.n,
          t = vecs.t,
          ids = participantIds,
    ...
    vecs.verify_fail_test_cases.foreach { vtc =>
      ...
      val pubshares =
        vtc.pubshare_indices.map(vecs.pubshares(_)).map(ECPublicKey.fromBytes)
      val pubnonces =
        vtc.pubnonce_indices.map(vecs.pubnonces(_)).map(FrostNoncePub.fromBytes)
  ...
  it must "pass sig_agg_vectors.json" in {
    ...
    val json = Json.parse(lines)
    val vecs = json.validate[SigAggVectors].get

    // Valid test cases: aggregate partial signatures and compare against expected
    vecs.valid_test_cases.foreach { tc =>
      ...
      val pubnonces =
        tc.pubnonce_indices.map(vecs.pubnonces(_)).map(FrostNoncePub.fromBytes)

      // Build signing context and session context to compute session values (b, R, e)
      val signingContext = FrostSigningContext(
        n = vecs.n,
        t = vecs.t,
        ids = participantIds,
      ...
      val sessionCtx = FrostSessionContext(
        ...
        tweaks =
          tc.tweak_indices.map(vecs.tweaks(_)).map(FieldElement.fromBytes),
        isXOnly = tc.is_xonly,
        message = vecs.msg
      )
      val signature = FrostUtil.partialSigAgg(
        partialSigs = tc.psigs.map(FieldElement.fromBytes),
        ids = participantIds,
        sessionContext = sessionCtx
      )
