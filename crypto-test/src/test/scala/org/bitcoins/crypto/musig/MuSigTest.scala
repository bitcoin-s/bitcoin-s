...
class MuSigTest extends BitcoinSCryptoTest {
  ...
  it should "work for a single party" in {
    forAll(CryptoGenerators.privateKey, NumberGenerator.bytevector(32)) {
      case (privKey, msg) =>
        ...
        val keySet = KeySet(Vector(pubKey), tweaks = Vector.empty)
        ...
        val sig = partialSigAgg(Vector(s), aggMuSigNoncePub, keySet, msg)
  ...
  it should "work for two parties" in {
    forAll(
      ...
      val keySet: KeySet = KeySet(Vector(pub1, pub2), tweaks = Vector.empty)
      ...
      val sig = partialSigAgg(Vector(s1, s2), aggMuSigNoncePub, keySet, msg)
      val aggPub = keySet.aggPubKey
  ...
  it should "work for more parties" in {
    ...
    forAll(
      ...
      val keySet: KeySet =
        KeySet(privKeysUnsorted.map(_.publicKey), tweaks = Vector.empty)
      ...
      val aggPub = keySet.aggPubKey
      val sig = partialSigAgg(partialSigs, aggMuSigNoncePub, keySet, msg)
