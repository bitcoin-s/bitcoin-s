...
class MuSigTest extends BitcoinSCryptoTest {
  ...
  it should "pass key aggregation test vectors" in {
    ...
    assert(
      UnsortedKeySet(
        Vector.fill(3)(inputs(0))
      ).aggPubKey.schnorrPublicKey == expected(2)
    )
    // Vector 4
    assert(
      UnsortedKeySet(
        Vector(inputs(0), inputs(0), inputs(1), inputs(1))
      ).aggPubKey.schnorrPublicKey == expected(3)
    )
  ...
  it should "pass tweak test vectors" in {
    ...
    // Vector 3
    val tweaks3 = Vector(
      MuSigTweak(tweaks(0), isXOnlyT = false),
    ...
    // Vector 4
    val tweaks4 = Vector(
      MuSigTweak(tweaks(0), isXOnlyT = true),
