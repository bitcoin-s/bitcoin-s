...
/** Indicates the services that are provided by a node on the P2P network
  ...
  * @see
  *   [[https://github.com/bitcoin/bitcoin/blob/master/src/protocol.h#L247 protocol.h]]
  *   in Bitcoin Core
  */
sealed abstract class ServiceIdentifier extends NetworkElement {
  ...
  /** This is a full node and can be asked for full blocks. It should implement
    * all protocol features available in its self-reported protocol version.
    */
  lazy val nodeNetwork: Boolean = reversedBits(0) // 1 << 0

  /** This is a full node capable of responding to the `getutxo` protocol
    * request. This is not supported by any currently-maintained Bitcoin node.
    *
    * @see
    *   [[https://github.com/bitcoin/bips/blob/master/bip-0064.mediawiki BIP64]]
  ...
  /** This is a full node capable and willing to handle bloom-filtered
    ...
    *
    * @see
    *   [[https://github.com/bitcoin/bips/blob/master/bip-0111.mediawiki BIP111]]
    *   for details
    */
  ...
  /** This is a full node that supports Xtreme Thinblocks. This is not supported
  ...
  /** NODE_COMPACT_FILTERS means the node will service basic block filter
    ...
    *
    * @see
    *   [[https://github.com/bitcoin/bips/blob/master/bip-0157 BIP157]] and
    *   [https://github.com/bitcoin/bips/blob/master/bip-0158 BIP158] for
    *   details on how this is implemented.
    *
    * @note
    *   This is not yet supported by any Core release. Currently (aug. 1 2019)
    *   is a open PR by jimpo: https://github.com/bitcoin/bitcoin/pull/16442
    */
  lazy val nodeCompactFilters: Boolean = reversedBits(6) // 1 << 6

  /** this means the same as `nodeNetwork` with the limitation of only serving
    * the last 288 (2 days) blocks
    *
    * @see
    *   [[https://github.com/bitcoin/bips/blob/master/bip-0159.mediawiki BIP159]]
    *   for details on how this is implemented.
    */
  ...
  /** returns true if this service identifier has at least all services
