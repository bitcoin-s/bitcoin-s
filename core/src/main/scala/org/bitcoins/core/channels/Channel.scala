...
sealed abstract class Channel {
  ...
  def outputIndex: Int = {
    ...
    require(
      outputOpt.isDefined,
      "We do not have the correct locking output on our anchor transasction")
    outputOpt.get._2
  }
  ...
  def scriptPubKey: P2WSHWitnessSPKV0 =
    lockingOutput.scriptPubKey.asInstanceOf[P2WSHWitnessSPKV0]
...
sealed trait ChannelAwaitingAnchorTx extends Channel {
  ...
  /**
    ...
    */
  def clientSign(
      clientChangeSPK: ScriptPubKey,
      amount: CurrencyUnit,
      privKey: ECPrivateKey)(
      implicit ec: ExecutionContext): Future[ChannelInProgressClientSigned] = {
    val invariants = Future {
      require(
        confirmations >= Policy.confirmations,
        "Need " + Policy.confirmations + " confirmations on the anchor tx before " +
          "we can create a payment channel in progress, got " + confirmations + " confirmations"
      )
      require(
        amount >= Policy.minChannelAmount,
        "First payment channel payment amount must be: " + Policy.minChannelAmount + " got: " + amount)
    }

    val o1 = TransactionOutput(lockedAmount - amount, clientChangeSPK)
    val outputs = Seq(o1)
    val partiallySigned = invariants.flatMap { _ =>
      EscrowTimeoutHelper.clientSign(outPoint,
                                     anchorTx,
                                     outputs,
                                     privKey,
                                     lock,
    ...
    val inProgress = partiallySigned.map(tx =>
      ChannelInProgressClientSigned(anchorTx, lock, clientChangeSPK, tx, Nil))
    inProgress
  }
  ...
  /**
    ...
    */
  def createClientSigned(
      partiallySigned: WitnessTransaction,
      clientChangeSPK: ScriptPubKey): Option[ChannelInProgressClientSigned] = {
    val inputOpt = partiallySigned.inputs.zipWithIndex
      .find(_._1.previousOutput.txId == anchorTx.txId)
    val inputIndex = inputOpt.map(i => UInt32(i._2))
    val txSigComponent = inputIndex.map { i =>
      WitnessTxSigComponent(partiallySigned,
                            i,
                            lockingOutput,
                            Policy.standardScriptVerifyFlags)
    }
    txSigComponent.map(t =>
      ChannelInProgressClientSigned(anchorTx, lock, clientChangeSPK, t, Nil))
  }
  ...
  def closeWithTimeout(
      ...
      fee: CurrencyUnit)(
      implicit ec: ExecutionContext): Future[ChannelClosedWithTimeout] = {
    ChannelClosedWithTimeout.closeWithTimeout(this,
                                              clientChangeSPK,
                                              clientKey,
                                              fee)
  }
...
sealed trait BaseInProgress { this: Channel =>
  ...
  def clientOutput: Option[TransactionOutput] =
    clientOutputIndex.map(idx => current.transaction.outputs(idx))
  ...
  /**
    ...
    */
  def serverAmount: Option[CurrencyUnit] =
    clientAmount.map(a => lockedAmount - a)

  /**
    * Attempts to close the [[Channel]] because the [[EscrowTimeoutScriptPubKey]]
    * has timed out
    */
  def closeWithTimeout(clientKey: ECPrivateKey, fee: CurrencyUnit)(
      implicit ec: ExecutionContext): Future[ChannelClosedWithTimeout] = {
    ChannelClosedWithTimeout.closeWithTimeout(this,
                                              clientChangeSPK,
                                              clientKey,
...
sealed trait ChannelInProgress extends Channel with BaseInProgress {
  ...
  def clientSign(amount: CurrencyUnit, clientKey: ECPrivateKey)(
      implicit ec: ExecutionContext): Future[ChannelInProgressClientSigned] = {
    ...
    val newClient: Try[Option[TransactionOutput]] = client match {
      case Some(c) =>
        ...
        if (newClientAmount < CurrencyUnits.zero)
          Failure(
            new IllegalArgumentException(
              "Client is spending more money than was locked in the output"))
      ...
      case None =>
        Failure(
          new IllegalArgumentException(
            "Client has already spent all of it's money to the server"))
    }
  ...
  private def updateChannel(
      ...
      clientKey: ECPrivateKey)(
      implicit ec: ExecutionContext): Future[ChannelInProgressClientSigned] = {
    val partiallySigned = EscrowTimeoutHelper.clientSign(
      outPoint,
      creditingTx,
      outputs,
      clientKey,
      lock,
      HashType.sigHashSingleAnyoneCanPay)
    val inProgress = partiallySigned.map(
      tx =>
        ChannelInProgressClientSigned(anchorTx,
                                      lock,
                                      clientChangeSPK,
                                      tx,
                                      current +: old))
  ...
  /**
    ...
    */
  def createClientSigned(
      partiallySigned: WitnessTransaction): ChannelInProgressClientSigned = {
    val txSigComponent = WitnessTxSigComponent(partiallySigned,
                                               current.inputIndex,
                                               lockingOutput,
                                               current.flags)
    ChannelInProgressClientSigned(anchorTx,
...
sealed trait ChannelInProgressClientSigned extends Channel with BaseInProgress {
  ...
  def serverSign(serverKey: ECPrivateKey)(
      implicit ec: ExecutionContext): Future[ChannelInProgress] = {
    ...
    val signedTxSigComponent: Future[WitnessTxSigComponentRaw] =
      EscrowTimeoutHelper.serverSign(serverKey,
                                     unsignedTxSigComponent,
  ...
  def close(
      serverSPK: ScriptPubKey,
      serverKey: ECPrivateKey,
      fee: CurrencyUnit)(
      implicit ec: ExecutionContext): Future[ChannelClosedWithEscrow] = {
    val c = clientOutput
    ...
    val outputs: Seq[TransactionOutput] =
      if (clientAmount <= Policy.dustThreshold || c.isEmpty) {
        Seq(serverOutput)
      } else {
    ...
    if (invariant.isFailure) {
      ...
      val updatedTx = WitnessTransaction(oldTx.version,
                                         oldTx.inputs,
                                         outputs,
                                         oldTx.lockTime,
                                         oldTx.witness)
      val txSigComponent = WitnessTxSigComponent(updatedTx,
                                                 current.inputIndex,
                                                 lockingOutput,
                                                 current.flags)
      val updatedInProgressClientSigned = ChannelInProgressClientSigned(
        anchorTx,
        lock,
        clientChangeSPK,
        txSigComponent,
...
object ChannelAwaitingAnchorTx {
  private case class ChannelAwaitAnchorTxImpl(
      anchorTx: Transaction,
      lock: EscrowTimeoutScriptPubKey,
      confirmations: Long)
      extends ChannelAwaitingAnchorTx {
    ...
    require(lockedAmount >= Policy.minChannelAmount,
            "We need to lock at least " + Policy.minChannelAmount +
  ...
  def apply(
      anchorTx: Transaction,
      lock: EscrowTimeoutScriptPubKey): Try[ChannelAwaitingAnchorTx] = {
  ...
  def apply(
      anchorTx: Transaction,
      lock: EscrowTimeoutScriptPubKey,
      confirmations: Long): Try[ChannelAwaitingAnchorTx] = {
...
object ChannelInProgress {
  private case class ChannelInProgressImpl(
      anchorTx: Transaction,
      lock: EscrowTimeoutScriptPubKey,
      clientChangeSPK: ScriptPubKey,
      current: WitnessTxSigComponent,
      old: Seq[WitnessTxSigComponent])
      extends ChannelInProgress

  def apply(
      anchorTx: Transaction,
      lock: EscrowTimeoutScriptPubKey,
      clientSPK: ScriptPubKey,
  ...
  def apply(
      ...
      clientSPK: ScriptPubKey,
      current: WitnessTxSigComponent,
      old: Seq[WitnessTxSigComponent]): ChannelInProgress = {
    ChannelInProgressImpl(anchorTx, lock, clientSPK, current, old)
  }
}

object ChannelInProgressClientSigned {
  private case class ChannelInProgressClientSignedImpl(
      anchorTx: Transaction,
      lock: EscrowTimeoutScriptPubKey,
      clientChangeSPK: ScriptPubKey,
      current: WitnessTxSigComponent,
      old: Seq[WitnessTxSigComponent])
      extends ChannelInProgressClientSigned
  ...
  def apply(
      ...
      clientChangeSPK: ScriptPubKey,
      current: WitnessTxSigComponent,
      old: Seq[WitnessTxSigComponent]): ChannelInProgressClientSigned = {
    ChannelInProgressClientSignedImpl(anchorTx,
                                      lock,
                                      clientChangeSPK,
                                      current,
                                      old)
...
object ChannelClosedWithEscrow {
  private case class ChannelClosedWithEscrowImpl(
      anchorTx: Transaction,
      lock: EscrowTimeoutScriptPubKey,
      current: WitnessTxSigComponent,
      old: Seq[WitnessTxSigComponent],
      clientChangeSPK: ScriptPubKey,
      serverSPK: ScriptPubKey)
      extends ChannelClosedWithEscrow {
    require(current.transaction.outputs.exists(_.scriptPubKey == serverSPK),
            "The final transaction must have a SPK that pays the server")
  }
  ...
  def apply(
      ...
      old: Seq[WitnessTxSigComponent],
      clientSPK: ScriptPubKey,
      serverSPK: ScriptPubKey): ChannelClosedWithEscrow = {
    ChannelClosedWithEscrowImpl(anchorTx,
                                lock,
                                current,
                                old,
                                clientSPK,
  ...
  def apply(
      i: ChannelInProgress,
      serverSPK: ScriptPubKey): ChannelClosedWithEscrow = {
    ChannelClosedWithEscrowImpl(i.anchorTx,
                                i.lock,
                                i.current,
                                i.old,
                                i.clientChangeSPK,
...
object ChannelClosedWithTimeout {
  private case class ChannelClosedWithTimeoutImpl(
      anchorTx: Transaction,
      lock: EscrowTimeoutScriptPubKey,
      current: WitnessTxSigComponent,
      old: Seq[WitnessTxSigComponent],
      clientChangeSPK: ScriptPubKey)
      extends ChannelClosedWithTimeout {
    require(
  ...
  def closeWithTimeout(
    ...
    val outputs = Seq(
      TransactionOutput(chan.lockedAmount - fee, clientChangeSPK))
    val outPoint =
      TransactionOutPoint(chan.anchorTx.txId, UInt32(chan.outputIndex))
    ...
    val signed: Future[WitnessTxSigComponent] =
      EscrowTimeoutHelper.closeWithTimeout(inputs,
                                           outputs,
                                           inputIndex,
                                           clientKey,
                                           chan.lock,
    ...
    signed.map(
      t =>
        ChannelClosedWithTimeout(chan.anchorTx,
                                 chan.lock,
                                 t,
                                 Nil,
                                 clientChangeSPK))
