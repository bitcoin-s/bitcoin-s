...
/** Represents an error that can be returned by the
  * [[org.bitcoins.core.wallet.builder.TxBuilder TxBuilder]] if it failed to
  * sign a set of utxos
  */
...
object TxBuilderError {
  ...
  /** This error indicates that the transaction failed to pass the invariants
    * the user wanted to hold true after the signing process was complete. An
    * example of this is the transaction is too big, or the fee level was too
    * high or low.
    */
  ...
  /** Means that we gave too many [[org.bitcoins.crypto.Sign Sign]] for the
    * TxBuilder to use during the signing process for a utxo. An example of this
    * occurring is if we gave 2 private keys to sign a p2pkh spk. A p2pkh only
    * requires one private key to sign the utxo.
    */
  ...
  /** Means that you are using the wrong
    * [[org.bitcoins.core.wallet.signer.Signer Signer]] to sign the given
    * [[org.bitcoins.core.protocol.script.ScriptPubKey ScriptPubKey]]
    */
  val WrongSigner = Failure(
    new IllegalArgumentException(
  ...
  /** Means that the
    * [[org.bitcoins.core.protocol.script.ScriptWitnessV0 ScriptWitnessV0]] you
    * passed as an argument does not hash to the commitment inside of
    * [[org.bitcoins.core.protocol.script.P2WSHWitnessSPKV0 P2WSHWitnessSPKV0]]
    */
  val WrongWitness = Failure(
    new IllegalArgumentException(
      "You passed in the wrong ScriptWitness type to sign the given WitnessScriptPubKey"
    )
  )

  /** Means that the redeem script you passed as an argument does not hash to
    * the commitment inside of the
    * [[org.bitcoins.core.protocol.script.P2SHScriptPubKey P2SHScriptPubKey]]
    */
  val WrongRedeemScript = Failure(
    new IllegalArgumentException(
  ...
  /** Can occurr when we are trying to sign a
    ...
    * [[org.bitcoins.core.protocol.script.ScriptWitness ScriptWitness]] for that
    * witness spk
    */
  ...
  /** Means that there is no signer defined for the given
    * [[org.bitcoins.core.protocol.script.ScriptPubKey ScriptPubKey]] type. An
    * example of a spk with no signer that is defined is
  ...
  /** Means that you specified a fee that was too large for the change output
    * you provided. This may happen if you have a transaction with a lot of
    * inputs, or the change output you provided is a output that contains a very
  ...
  /** Means that the
    ...
    * outputs you specified when creating the
    * [[org.bitcoins.core.wallet.builder.TxBuilder TxBuilder]] are NOT all
    * included in the final signed tx
    */
  ...
  /** Means that the script we are signing for requires a public key, but we did
    * not pass one in as a parameter inside of [[org.bitcoins.crypto.Sign Sign]]
    */
  ...
  /** Means that the signed version of this transaction has MORE outputs than
    * what was specified when building the
    * [[org.bitcoins.core.wallet.builder.TxBuilder TxBuilder]].
    * [[org.bitcoins.core.wallet.builder.TxBuilder.destinations TxBuilder.destinations]]
    * &&
    * [[org.bitcoins.core.wallet.builder.TxBuilder.changeSPK TxBuilder.changeSPK]]
    * should be the only outputs in the signedTx
    */
  val ExtraOutputsAdded = Failure(
    new IllegalArgumentException(
      "More outputs were added to the transaction than were initally passed in"
    )
  )

  /** Means that the transaction spends outpoints that were not given when
    * creating the [[org.bitcoins.core.wallet.builder.TxBuilder TxBuilder]],
    * aka, we should only spend outpoints in
    * [[org.bitcoins.core.wallet.builder.TxBuilder.outPoints TxBuilder.outPoints]]
    */
  val ExtraOutPoints = Failure(
    new IllegalArgumentException(
  ...
  /** Indicates we are spending multiple
    ...
    * and that one of those spk's outputs are locked by block height, while the
    * other is locked by a time stamp. Since there is only one locktime field on
    * a transaction, we cannot satisfy both of these locktimes simultaneously.
    */
  val IncompatibleLockTimes = Failure(
    new IllegalArgumentException(
  ...
  def OutputBelowDustThreshold(
      belowDustOutputs: Seq[TransactionOutput]
  ): Failure[Nothing] =
    Failure(
      new IllegalArgumentException(
