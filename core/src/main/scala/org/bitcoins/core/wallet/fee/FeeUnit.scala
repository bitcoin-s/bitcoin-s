...
/** This is meant to be an abstract type that represents different fee unit
  * measurements for blockchains
  */
sealed abstract class FeeUnit {
  ...
  /** The coefficient the denominator in the unit is multiplied by, for example
    * sats/kilobyte -> 1000
    */
  ...
  /** Takes the given transaction returns a size that will be used for
    * calculating the fee rate. This is generally the denominator in the unit,
    * ie sats/byte
    */
  def txSizeForCalc(tx: Transaction): Long = factory.txSizeForCalc(tx)

  /** Calculates the fee for the transaction using this fee rate, rounds down
    * satoshis
    */
...
trait FeeUnitFactory[+T <: FeeUnit] {
  ...
  /** Creates an instance T where the value given is the numerator of the fee
    * unit
    */
  def fromLong(long: Long): T

  /** Calculates the fee rate from the given Transaction. Uses Math.round for
    * rounding up, which returns the closest long to the argument
...
/** A 'virtual byte' (also known as virtual size) is a new weight measurement
  * that was created with segregated witness (BIP141). Now 1 'virtual byte' has
  * the weight of 4 bytes in the
  * [[org.bitcoins.core.protocol.transaction.TransactionWitness]] of a
...
object SatoshisPerVirtualByte extends FeeUnitFactory[SatoshisPerVirtualByte] {
  ...
  /** Used to indicate we could not retrieve a fee from a
    * [[org.bitcoins.core.api.feeprovider.FeeRateApi]]
    */
...
/** Weight is used to indicate how 'expensive' the transaction is on the
  * blockchain. This use to be a simple calculation before segwit (BIP141). Each
  * byte in the transaction counted as 4 'weight' units. Now with segwit, the
