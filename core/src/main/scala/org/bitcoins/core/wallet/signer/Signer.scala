...
sealed abstract class RawSingleKeyBitcoinSigner[-InputType <: RawInputInfo]
  ...
  def keyAndSigToScriptSig(
      ...
      spendingInfo: InputSigningInfo[InputType]
  ): ScriptSignature
  ...
  override def sign(
    ...
    val scriptSig =
      keyAndSigToScriptSig(
        partialSignature.pubKey.toPublicKey,
...
sealed abstract class P2PKSigner
  ...
  override def keyAndSigToScriptSig(
      key: ECPublicKey,
      sig: ECDigitalSignature,
      spendingInfo: InputSigningInfo[P2PKInputInfo]
  ): ScriptSignature = {
...
sealed abstract class P2PKHSigner
  ...
  override def keyAndSigToScriptSig(
      key: ECPublicKey,
      sig: ECDigitalSignature,
      spendingInfo: InputSigningInfo[P2PKHInputInfo]
  ): ScriptSignature = {
    ...
    if (spendingInfoToSatisfy != spendingInfo) {
      ...
      unsignedTx match {
        case wtx: WitnessTransaction =>
          ...
          val unsignedWtx = WitnessTransaction(
            wtx.version,
            wtx.inputs,
            wtx.outputs,
            wtx.lockTime,
          ...
          val signedTx = WitnessTransaction(
            unsignedWtx.version,
            unsignedWtx.inputs,
            unsignedWtx.outputs,
...
sealed abstract class P2WSHSigner extends Signer[P2WSHV0InputInfo] {
  ...
  override def sign(
    ...
    if (spendingInfoToSatisfy != spendingInfo) {
      ...
      val signedSigComponent = BitcoinSigner.sign(
        spendingInfo,
        wtx,
        isDummySignature,
        nestedSpendingInfo
      ...
      val signedWTx = WitnessTransaction(
        wtx.version,
        wtx.inputs,
        wtx.outputs,
        wtx.lockTime,
...
sealed abstract class ConditionalSigner extends Signer[ConditionalInputInfo] {
  ...
  override def sign(
    ...
    val missingOpSigComponent = BitcoinSigner.sign(
      spendingInfo,
      unsignedTx,
      isDummySignature,
      nestedSpendingInfo
    ...
    val scriptSig =
      ConditionalScriptSignature(
        missingOpSigComponent.scriptSignature,
