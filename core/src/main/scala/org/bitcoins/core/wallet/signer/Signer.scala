...
sealed abstract class Signer {
  ...
  def doSign(
      ...
      sign: Seq[Byte] => Future[ECDigitalSignature],
      hashType: HashType,
      isDummySignature: Boolean)(
          ...
          signature.flatMap { s =>
            redeemScript.map { rs =>
              ...
              val signedWitness = TransactionWitness(
                wtx.witness.witnesses.updated(inputIndex.toInt, scriptWit))
    ...
    if (signers.size != 1) {
      ...
      val signed: Future[TxSigComponent] = spk match {
        case p2wshSPK: P2WSHWitnessSPKV0 =>
          ...
          scriptWit.map { wit =>
            val txWit = TransactionWitness(
              wtx.witness.witnesses.updated(inputIndex.toInt, wit))
            val signedWTx = WitnessTransaction(wtx.version,
                                               wtx.inputs,
                                               wtx.outputs,
                                               wtx.lockTime,
        ...
        if (signers.size != 1) {
          ...
          witSPK.flatMap { w =>
            ...
            signature.map { sig =>
              ...
              val signedTxWitness = TransactionWitness(
                unsignedWtx.witness.witnesses.updated(inputIndex.toInt,
                                                      scriptWitness))
