...
trait HDPath extends BIP32Path {
  ...
  /** This type is to give a cleaner return type of `next`.
    *
    * Consider:
    *
    * {{{
  ...
  /** Increments the address index and returns the new path that can be passed
    * into a [[org.bitcoins.core.crypto.ExtKey ExtKey]]
    */
  def next: NextPath =
