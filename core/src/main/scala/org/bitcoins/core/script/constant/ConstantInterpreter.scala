...
sealed abstract class ConstantInterpreter {
  ...
  def opPushData1(
    ...
    require(
      program.script.headOption.contains(OP_PUSHDATA1),
  ...
  def opPushData4(
    ...
    require(
      program.script.headOption.contains(OP_PUSHDATA4),
  ...
  def pushScriptNumberBytesToStack(
    ...
    val constant: ScriptToken =
      if (bytesToPushOntoStack.size == 1) bytesToPushOntoStack.head
        ...
        ScriptConstant(
          BytesUtil.flipEndianness(BytesUtil.toByteVector(bytesToPushOntoStack))
        )
      ...
      case _: ScriptToken =>
        if (
          ScriptFlagUtil.requireMinimalData(
            program.flags
          ) && program.script.size > 2 && !BitcoinScriptUtil
