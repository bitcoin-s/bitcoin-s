...
/** If the top stack value is not 0, the statements are executed. The top stack
  * value is removed.
  */
...
/** Marks transaction as invalid. A standard way of attaching extra data to
  * transactions is to add a zero-value output with a scriptPubKey consisting of
  * `OP_RETURN` followed by exactly one pushdata op. Such outputs are provably
  ...
  * one `OP_RETURN` output or an `OP_RETURN` output with more than one pushdata
  * op.
  */
...
object ControlOperations extends ScriptOperationFactory[ControlOperations] {
  ...
  override val operations: Vector[ControlOperations] =
