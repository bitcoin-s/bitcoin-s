...
sealed abstract class ScriptInterpreter {
        ...
        if (scriptSigExecutedProgram.error.isDefined) {
          ...
          scriptPubKey match {
            ...
            case p2sh: P2SHScriptPubKey =>
              if (p2shEnabled)
                executeP2shScript(scriptSigExecutedProgram, program, p2sh)
              else scriptPubKeyExecutedProgram
  ...
  private def executeP2shScript(
      ...
      originalProgram: ScriptProgram,
      p2shScriptPubKey: P2SHScriptPubKey): ExecutedScriptProgram = {
          ...
          redeemScript match {
            case w: WitnessScriptPubKey =>
              ...
              if (segwitEnabled && (scriptSig.asmBytes == expectedScriptBytes)) {
                ...
                logger.error(
                  "expected scriptsig bytes: " + BitcoinSUtil.encodeHex(
                    expectedScriptBytes))
  ...
  private def loop(
    ...
    } else if (program.script.flatMap(_.bytes).size > 10000 && !program
                 .isInstanceOf[ExecutedScriptProgram]) {
