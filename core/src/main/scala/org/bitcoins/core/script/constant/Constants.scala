...
object ScriptNumber extends Factory[ScriptNumber] {
  ...
  def apply(bytes: Seq[Byte], requireMinimal: Boolean): Try[ScriptNumber] =
  ...
  /**
    ...
    */
  private case class ScriptNumberImpl(
      underlying: Long,
  ...
  private object ScriptNumberImpl {
    ...
    def apply(hex: String): ScriptNumber =
      ScriptNumberImpl(ScriptNumberUtil.toLong(hex), hex)

    def apply(bytes: Seq[Byte]): ScriptNumber =
      ScriptNumberImpl(ScriptNumberUtil.toLong(bytes))

    def apply(underlying: Long): ScriptNumber =
      ScriptNumberImpl(underlying, ScriptNumberUtil.longToHex(underlying))
...
object ScriptConstant extends Factory[ScriptConstant] {
  ...
  def fromBytes(bytes: Seq[Byte]): ScriptConstant =
    ScriptConstantImpl(BitcoinSUtil.encodeHex(bytes))
