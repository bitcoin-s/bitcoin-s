...
trait BitcoinScriptUtil extends BitcoinSLogger {
  ...
  def asmToBytes(asm: Seq[ScriptToken]): Seq[Byte] =
  ...
  def isPushOnly(script: Seq[ScriptToken]): Boolean = {
    @tailrec
    def loop(tokens: Seq[ScriptToken], accum: List[Boolean]): Seq[Boolean] =
      tokens match {
        case h :: t =>
          h match {
            case scriptOp: ScriptOperation =>
              loop(t, (scriptOp.opCode < OP_16.opCode) :: accum)
  ...
  def calculatePushOp(bytes: Seq[Byte]): Seq[ScriptToken] =
