...
sealed abstract class ScriptParser
  ...
  /** Parses an asm output script of a transaction example: "OP_DUP OP_HASH160
    * e2e7c1ab3f807151e832dd1accb3d4f5d7d19b4b OP_EQUALVERIFY OP_CHECKSIG"
    * example: ["0", "IF 0x50 ENDIF 1", "P2SH,STRICTENC", "0x50 is reserved (ok
    * if not executed)"] (from script_valid.json)
    */
  ...
  /** Parses a string to a sequence of script tokens example: "OP_DUP OP_HASH160
    * e2e7c1ab3f807151e832dd1accb3d4f5d7d19b4b OP_EQUALVERIFY OP_CHECKSIG"
    * example: ["0", "IF 0x50 ENDIF 1", "P2SH,STRICTENC", "0x50 is reserved (ok
    * if not executed)"] (from script_valid.json)
    */
  ...
  /** Parses a byte array into a the asm operations for a script will throw an
  ...
  /** Slices the amount of bytes specified in the bytesToPushOntoStack parameter
    * and then creates a script constant from those bytes. Returns the script
    * constant and the byte array without the script constant
    */
  ...
  /** Parses the bytes in string format, an example input would look like this
    * "0x09 0x00000000 0x00000000 0x10" see
    * [[https://github.com/bitcoin/bitcoin/blob/master/src/test/data/script_valid.json#L21-L25]]
  ...
  /** Parses an operation if the tail is a scodec.bits.ByteVector If the
    ...
    * BytesToPushOntoStackImpl(5), it would slice 5 bytes off of the tail and
  ...
  /** Parses OP_PUSHDATA operations correctly. Slices the appropriate amount of
    ...
    * @param accum
    *   the parsed script tokens so far
    * @param tail
    *   the bytes to be parsed still
    * @return
    */
  ...
  /** Helper function to build the parsing helper for parsing an OP_PUSHDATA
    * operation
    * @param op
    *   the OP_PUSHDATA operation being added to the accum
    * @param bytesToPushOntoStack
    *   the number of bytes that are pushed onto the stack by the OP_PUSHDATA
    *   operation
    * @param scriptConstant
    *   the constant that is being pushed onto the stack by the OP_PUSHDATA
    *   operation
    * @param restOfBytes
    *   the remaining bytes that need to be parsed
    * @param accum
    *   the accumulator filled with script tokens that have already been parsed
    * @return
    */
  private def buildParsingHelper(
    ...
    if (bytesToPushOntoStack.bytes == ByteVector.fromByte(0)) {
