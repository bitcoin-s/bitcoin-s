...
trait RawNetworkHeaderSerializer extends RawBitcoinSerializer[NetworkHeader] {
  ...
  /** Transforms a sequence of bytes into a message header
    * @param bytes
    *   the byte representation for a MessageHeader on the peer-to-peer network
    * @return
    *   the native object for the MessageHeader
    */
  ...
  /** Takes in a message header and serializes it to hex
    * @param messageHeader
    *   the message header to be serialized
    * @return
    *   the hexadecimal representation of the message header
    */
