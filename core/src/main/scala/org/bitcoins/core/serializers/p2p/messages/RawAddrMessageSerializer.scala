...
trait RawAddrMessageSerializer extends RawBitcoinSerializer[AddrMessage] {
  ...
  /** Parses ip addresses inside of an AddrMessage
    * @param ipCount
    *   the number of ip addresses we need to parse from the AddrMessage
    * @param bytes
    *   the bytes from which we need to parse the ip addresses
    * @return
    *   the parsed ip addresses and the remaining bytes
    */
