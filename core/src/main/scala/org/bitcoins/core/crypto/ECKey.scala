...
sealed abstract class BaseECKey extends NetworkElement with Sign {
  ...
  override def signFunction: Seq[Byte] => Future[ECDigitalSignature] = {
    bytes =>
  ...
  override def sign(dataToSign: Seq[Byte]): ECDigitalSignature =
...
sealed abstract class ECPrivateKey extends BaseECKey {
  ...
  def publicKey: ECPublicKey = {
    val pubKeyBytes: Seq[Byte] =
      NativeSecp256k1.computePubkey(bytes.toArray, isCompressed)
    require(
      NativeSecp256k1.isValidPubKey(pubKeyBytes.toArray),
      "secp256k1 failed to generate a valid public key, got: " + BitcoinSUtil
        .encodeHex(pubKeyBytes))
  ...
  def toWIF(network: NetworkParameters): String = {
    ...
    val fullBytes =
      if (isCompressed) networkByte ++ (bytes ++ Seq(1.toByte))
...
object ECPrivateKey extends Factory[ECPrivateKey] {
  ...
  def apply(bytes: Seq[Byte], isCompressed: Boolean)(
...
sealed abstract class ECPublicKey extends BaseECKey {
  ...
  def decompressed: ECPublicKey = {
    if (isCompressed)
      ECPublicKey.fromBytes(NativeSecp256k1.decompress(bytes.toArray))
    else this
  }
}
