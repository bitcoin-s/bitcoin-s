...
trait TransactionConstants {
  ...
  /** If bit (1 << 31) of the sequence number is set, then no consensus meaning
    * is applied to the sequence number and can be included in any block under
    * all currently possible circumstances.
    * @return
    *   the mask that ben used with a bitwise and to indicate if the sequence
    *   number has any meaning
    */
  def locktimeDisabledFlag: UInt32 = UInt32(1L << 31)

  /** If a transaction's input's sequence number encodes a relative lock-time,
  ...
  /** If the transaction input sequence number encodes a relative lock-time and
    * this flag is set, the relative lock-time has units of 512 seconds,
    * otherwise it specifies blocks with a granularity of 1.
    */
  def sequenceLockTimeTypeFlag: UInt32 = UInt32(1L << 22)

  /** Threshold for nLockTime: below this value it is interpreted as block
