...
sealed abstract class P2WPKHWitnessV0 extends ScriptWitness {
  ...
  override def toString =
    "P2WPKHWitnessV0(" + stack.map(BitcoinSUtil.encodeHex(_)).toString + ")"
}

object P2WPKHWitnessV0 {
  private case class P2WPKHWitnessV0Impl(stack: Seq[Seq[Byte]])
      extends P2WPKHWitnessV0

  private def apply(stack: Seq[Seq[Byte]]): P2WPKHWitnessV0 =
...
sealed abstract class P2WSHWitnessV0 extends ScriptWitness {
  ...
  override def toString =
    "P2WSHWitnessV0(" + stack.map(BitcoinSUtil.encodeHex(_)).toString + ")"
}

object P2WSHWitnessV0 {
  private case class P2WSHWitnessV0Impl(stack: Seq[Seq[Byte]])
...
object ScriptWitness {
  ...
  def apply(stack: Seq[Seq[Byte]]): ScriptWitness = {
    ...
    val isPubKey = stack.headOption.isDefined && ECPublicKey.isFullyValid(
      stack.head) && (stack.head.size == 33 || stack.head.size == 65)
    if (stack.isEmpty) {
      EmptyScriptWitness
