...
/** Created by chris on 8/7/16. Represents a subset of known txids inside of a
  ...
  * recovery of the txids & merkle root without having to store them all
  * explicitly. See
  ...
  * This bit signifies if the node is a parent of at least one matched leaf txid
  * (or a matched leaf txid) itself. In case we are the leaf level, or this bit
  * is 0, it's merkle node hash is stored and it's children are not explored any
  * further. Otherwise no hash is stored, but we recurse all of this node's
  * child branches.
  *
  * Decoding procedure:
...
sealed trait PartialMerkleTree {
  ...
  /** A sequence representing if this node is the parent of another node that
    * matched a txid
    */
  def bits: BitVector
        ...
        if (remainingBits.head) {
          ...
          subTree match {
            case n: Node[DoubleSha256Digest] =>
              ...
              if (
                PartialMerkleTree.existsRightSubTree(
                  pos,
                  numTransactions,
                  maxHeight,
                  height
  ...
  /** Handles a leaf match when we are extracting matches from the partial
    * merkle tree
    */
...
object PartialMerkleTree {
  ...
  /** @param txMatches
    *   indicates whether the given txid matches the bloom filter, the full
    *   merkle branch needs to be included inside of the
    *   [[org.bitcoins.core.protocol.blockchain.PartialMerkleTree PartialMerkleTree]]
    * @return
    *   the binary tree that represents the partial merkle tree, the bits needed
  ...
  private def build(
    ...
    /** This loops through our merkle tree building `bits` so we can instruct
      * another node how to create the partial merkle tree
      * @see
      *   [[https://github.com/bitcoin/bitcoin/blob/b7b48c8bbdf7a90861610b035d8b0a247ef78c45/src/merkleblock.cpp#L78]]
      * @param bits
      *   the accumulator for bits indicating how to reconsctruct the partial
      *   merkle tree
      * @param hashes
      *   the relevant hashes used with bits to reconstruct the merkle tree
      * @param height
      *   the transaction index we are currently looking at -- if it was matched
      *   in our bloom filter we need the entire merkle branch
      * @param pos
      *   the position in the loop represented as an int
      * @return
      *   the binary tree that represents the partial merkle tree, the bits
      *   needed to reconstruct this partial merkle tree, and the hashes needed
      *   to be inserted according to the flags inside of bits
      */
    def loop(
        bits: BitVector,
        hashes: Seq[DoubleSha256Digest],
  ...
  /** Checks if a node at given the given height and position matches a
  ...
  def matchesTx(
      ...
      height: Int,
      pos: Int,
      matchedTx: Seq[(Boolean, DoubleSha256Digest)]
  ): Boolean = {
  ...
  /** This constructor creates a partial from this given
    * [[org.bitcoins.core.util.BinaryTree BinaryTree]] You probably don't want
    * to use this constructor, unless you manually constructed `bits` and the
    * `tree` by hand
    * @param tree
  ...
  private def reconstruct(
    ...
    def loop(
        ...
        height: Int,
        pos: Int
    ): (BinaryTree[DoubleSha256Digest], Seq[DoubleSha256Digest], BitVector) = {
      if (height == maxHeight) {
  ...
  /** Determines if the right sub tree can exists inside of the partial merkle
    ...
    * exists when we are building a partial merkle tree from bottom up, NOT TOP
    * DOWN. If we are building a tree from top down use it's counterpart that
    * does NOT take a maxHeight parameter
    */
  ...
  /** Determines if the right sub tree can exist inside of the partial merkle
    * tree
    */
  private def existsRightSubTree(
      pos: Int,
