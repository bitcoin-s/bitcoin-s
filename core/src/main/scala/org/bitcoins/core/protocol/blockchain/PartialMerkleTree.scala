...
object PartialMerkleTree {
  ...
  private def build(txMatches: Seq[(Boolean, DoubleSha256Digest)]): (
    ...
    def loop(
        ...
        hashes: Seq[DoubleSha256Digest],
        height: Int,
        pos: Int): (Seq[Boolean], Seq[DoubleSha256Digest]) = {
      val parentOfMatch =
  ...
  def apply(
      transactionCount: UInt32,
      hashes: Seq[DoubleSha256Digest],
      bits: Seq[Boolean]): PartialMerkleTree = {
  ...
  private def usedAllBits(
      bits: Seq[Boolean],
      remainingBits: Seq[Boolean]): Boolean = {
