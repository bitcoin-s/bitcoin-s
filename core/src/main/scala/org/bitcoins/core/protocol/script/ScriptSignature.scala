...
object NonStandardScriptSignature
  ...
  private case class NonStandardScriptSignatureImpl(bytes: Seq[Byte])
...
object P2PKHScriptSignature extends ScriptFactory[P2PKHScriptSignature] {
  private case class P2PKHScriptSignatureImpl(bytes: Seq[Byte])
  ...
  def isP2PKHScriptSig(asm: Seq[ScriptToken]): Boolean = asm match {
    case List(w: BytesToPushOntoStack,
              x: ScriptConstant,
              y: BytesToPushOntoStack,
              z: ScriptConstant) =>
...
sealed trait P2SHScriptSignature extends ScriptSignature {
  ...
  def redeemScript: ScriptPubKey = {
    ...
    //see BIP141
    WitnessScriptPubKey(asm).getOrElse(
  ...
  def signatures: Seq[ECDigitalSignature] =
    scriptSignatureNoRedeemScript match {
      case Failure(_) => Nil
      case Success(nonRedeemScript) =>
        val sigs = nonRedeemScript.asm
          .filter(_.isInstanceOf[ScriptConstant])
          .filterNot(_.isInstanceOf[ScriptNumberOperation])
  ...
  /**
    ...
    */
  def splitAtRedeemScript(
      asm: Seq[ScriptToken]): Try[(Seq[ScriptToken], Seq[ScriptToken])] = {
...
object P2SHScriptSignature extends ScriptFactory[P2SHScriptSignature] {
  private case class P2SHScriptSignatureImpl(bytes: Seq[Byte])
...
object MultiSignatureScriptSignature
  ...
  private case class MultiSignatureScriptSignatureImpl(bytes: Seq[Byte])
...
object P2PKScriptSignature extends ScriptFactory[P2PKScriptSignature] {
  private case class P2PKScriptSignatureImpl(bytes: Seq[Byte])
...
object CLTVScriptSignature extends Factory[CLTVScriptSignature] {
  private case class CLTVScriptSignatureImpl(bytes: Seq[Byte])
...
object CSVScriptSignature extends Factory[CSVScriptSignature] {
  private case class CSVScriptSignatureImpl(bytes: Seq[Byte])
  ...
  def fromScriptPubKey(
    ...
    scriptPubKey match {
      ...
      case escrowWithTimeout: EscrowTimeoutScriptPubKey =>
        ...
        if (isMultiSig) {
          val multiSig = Try(
            MultiSignatureScriptSignature.fromAsm(tokens.take(tokens.size - 1)))
          multiSig.map(m => EscrowTimeoutScriptSignature.fromMultiSig(m))
        } else
          Try(EscrowTimeoutScriptSignature.fromAsm(tokens, escrowWithTimeout))
...
object EscrowTimeoutScriptSignature
    extends Factory[EscrowTimeoutScriptSignature] {
  private case class EscrowTimeoutScriptSignatureImpl(bytes: Seq[Byte])
      extends EscrowTimeoutScriptSignature
  ...
  def fromAsm(
      asm: Seq[ScriptToken],
      scriptPubKey: EscrowTimeoutScriptPubKey): EscrowTimeoutScriptSignature = {
    require(isEscrowTimeoutScriptSig(asm, Some(scriptPubKey)),
            "Given asm was not a EscrowWithTimeoutScriptSignature, got: " + asm)
  ...
  def fromAsm(asm: Seq[ScriptToken]): EscrowTimeoutScriptSignature = {
    ...
    val nestedScriptSig = if (BitcoinScriptUtil.castToBool(asm.last)) {
      require(
        ...
        "Need multisigScriptSig, got: " + nested)
  ...
  def isEscrowTimeoutScriptSig(
      asm: Seq[ScriptToken],
      scriptPubKey: Option[EscrowTimeoutScriptPubKey] = None): Boolean = {
    val nested = asm.take(asm.length - 1)
    val last = asm.last
  ...
  def apply(scriptSig: ScriptSignature): Try[EscrowTimeoutScriptSignature] =
    scriptSig match {
      case m: MultiSignatureScriptSignature => Success(fromMultiSig(m))
      case lock: LockTimeScriptSignature    => Success(fromLockTime(lock))
      case x @ (_: P2PKScriptSignature | _: P2PKHScriptSignature |
          _: P2SHScriptSignature | _: NonStandardScriptSignature |
          _: EscrowTimeoutScriptSignature | EmptyScriptSignature) =>
        Failure(
          new IllegalArgumentException(
            "Cannot create a EscrowTimeoutScriptSignature out of " + x))
  ...
  /**
    ...
    */
  def fromMultiSig(
      multiSigScriptSig: MultiSignatureScriptSignature): EscrowTimeoutScriptSignature = {
