...
sealed trait BlockHeader extends NetworkElement {
  ...
  /** The block version number indicates which set of block validation rules to
    * follow. See the list of block versions below.
    *
    * @see
    *   BIP9 for more information on what version number signify
  ...
  /** Returns the big endian encoding of the previous block hash This is useful
    * for using rpc and block exporers, but is NOT used in the protocol itself
    *
    * @see
    *   see this Stack Exchange question for more:
  ...
  /** A `SHA256(SHA256())` hash in internal byte order. The merkle root is
    ...
    * ensuring that none of those transactions can be modified without modifying
    * the header.
    *
    * @see
  ...
  /** Returns the merkle root hash in BIG ENDIAN format. This is not compatible
    * with the bitcoin protocol but it is useful for rpc clients and block
    * explorers
    *
    * @see
  ...
  /** The block time is a Unix epoch time when the miner started hashing the
    * header (according to the miner). Must be greater than or equal to the
    * median time of the previous 11 blocks. Full nodes will not accept blocks
    * with headers more than two hours in the future according to their clock.
    *
    * @return
  ...
  /** This is the decoded version of [[nBits]]. nBits is used to compactly
    ...
    * This is a 256 bit unsigned integer See the bitcoin developer reference for
  ...
  /** An arbitrary number miners change to modify the header hash in order to
    * produce a hash below the target threshold. If all 32-bit values are
    * tested, the time can be updated or the coinbase transaction can be changed
    * and the merkle root updated.
    *
    * @return
  ...
  /** Returns the block hash in big endian format, this is useful for rpc and
    * block explorer debugging. This is *not* used in the core protocol itself.
...
object BlockHeader extends Factory[BlockHeader] {
  ...
  /** Return type used to carry around extra information about the difficulty
    * required to mine a block. Unfortunately there is weird corner cases like
