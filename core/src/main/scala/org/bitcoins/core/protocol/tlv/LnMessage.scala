...
/** Lightning Network message serialization is the same as TLV serialization
  * except that the type is represented with a UInt16 instead of BigSizeUInt and
  * length is omitted.
  *
  * The reason for the omission is that the message is expected to be encrypted
  * by the LN transport layer and the length is included there in the
  * unencrypted part of the packet.
  *
  * The reason that LnMessage doesn't just do what TLV does (which is better) is
  * because TLVs are newer and so we're stuck with the legacy format.
  */
