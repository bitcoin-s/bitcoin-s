...
object TransactionWitness {
  private case class TransactionWitnessImpl(witnesses: Seq[ScriptWitness])
  ...
  def fromBytes(bytes: Seq[Byte], numInputs: Int): TransactionWitness =
    RawTransactionWitnessParser.read(bytes, numInputs)

  def apply(bytes: Seq[Byte], numInputs: Int): TransactionWitness =
