...
/** Created by chris on 11/21/16. The witness data for
  * [[org.bitcoins.core.protocol.script.ScriptSignature ScriptSignature]] in
  * this transaction
...
sealed abstract class TransactionWitness
  ...
  /** Update the `witnesses` index to the given witness Pads the witnesses
    * vector if needed to accomodate the new witness
    */
  ...
  /** Pads the existing `witnesses` so that we can insert a witness into the
    * given index. If `witnesses` is not properly padded we can have an index
    * out of bounds exception thrown when trying to update the vector.
    */
...
/** Each input (even if it does not spend a segwit output) needs to have a
  * witness associated with it in a [[WitnessTransaction]]. This helper case
  * class is used to "fill in" [[EmptyScriptWitness]] for the inputs that do not
  * spend a
...
object TransactionWitness {
  ...
  /** Creates a
    ...
    * [[org.bitcoins.core.protocol.script.EmptyScriptWitness EmptyScriptWitness]]
    * and then calls the normal
    * [[org.bitcoins.core.protocol.transaction.TransactionWitness TransactionWitness]]
    * constructor
    */
  def fromWitOpt(
