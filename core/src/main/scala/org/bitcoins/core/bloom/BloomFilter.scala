...
/** Implements a bloom filter that abides by the semantics of BIP37
  ...
  * @see
  *   [[https://github.com/bitcoin/bitcoin/blob/master/src/bloom.h Bitcoin Core bloom.h]]
  */
sealed abstract class BloomFilter extends NetworkElement {
  ...
  /** An arbitrary value to add to the seed value in the hash function used by
    * the bloom filter.
    */
  def tweak: UInt32

  /** A set of flags that control how outpoints corresponding to a matched
    * pubkey script are added to the filter. See the 'Comparing Transaction
    * Elements to a Bloom Filter' section in this
    * @see
    *   [[https://bitcoin.org/en/developer-reference#filterload link]]
    */
  def flags: BloomFlag

  /** Inserts a sequence of bytes into the
  ...
  /** Inserts a public key and it's corresponding hash into the bloom filter
    *
    * @note
    *   The rationale for inserting both the pubkey and its hash is that in most
    *   cases where you have an "interesting pubkey" that you want to track on
    *   the P2P network what you really need to do is insert the hash of the
    *   public key, as that is what appears on the blockchain and that nodes you
    *   query with bloom filters will see.
    *
    * @see
    *   The
  ...
  /** Checks if the transaction's txid, or any of the constants in it's
    * scriptPubKeys/scriptSigs match our BloomFilter
    *
    * @see
    *   [[https://github.com/bitcoin/bips/blob/master/bip-0037.mediawiki#filter-matching-algorithm BIP37]]
    *   for exact details on what is relevant to a bloom filter and what is not
    *   relevant
    */
  ...
  /** Updates this bloom filter to contain the relevant information for the
    * given Transaction
    *
    * @see
    *   [[https://github.com/bitcoin/bips/blob/master/bip-0037.mediawiki#filter-matching-algorithm BIP37]]
    *   for the exact details on what parts of a transaction is added to the
    *   bloom filter
    */
  ...
  /** Performs the [[scala.util.hashing.MurmurHash3 MurmurHash3]] on the given
    ...
    *
    * @param hashNum
    *   the nth hash function we are using
    * @param bytes
    *   the bytes of the data that needs to be inserted into the
    *   [[org.bitcoins.core.bloom.BloomFilter BloomFilter]]
    * @return
    *   the index of the bit inside of `data` that needs to be set to 1
    */
  ...
  /** Adds a sequence of byte vectors to our bloom filter then returns that new
    * filter
    */
...
object BloomFilter extends Factory[BloomFilter] {
  ...
  /** Creates a bloom filter based on the number of elements to be inserted into
    ...
    *
    * @param tweak
    *   A random value that only acts to randomize the filter and increase
    *   privacy
    */
  def apply(
      numElements: Int,
