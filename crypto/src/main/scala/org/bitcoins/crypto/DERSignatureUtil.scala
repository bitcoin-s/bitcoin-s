...
sealed abstract class DERSignatureUtil {
  ...
  /** Checks if this signature is encoded to DER correctly
    ...
    * NOTE: This will fail if this signature contains the hash type appended to
    * the end of it
    * @return
  ...
  /** Checks if the bytes are encoded to DER correctly
    * https://crypto.stackexchange.com/questions/1795/how-can-i-convert-a-der-ecdsa-signature-to-asn-1
    * This will fail if this signature contains the hash type appended to the
    * end of it
    * @return
    *   boolean representing if the signature is a valid
    */
  ...
  /** Decodes the given sequence of bytes into it's r and s points throws an
    * exception if the given sequence of bytes is not a DER encoded signature
    */
  ...
  /** This functions implements the strict der encoding rules that were created
    * in BIP66
    * [[https://github.com/bitcoin/bips/blob/master/bip-0066.mediawiki]]
    * [[https://github.com/bitcoin/bitcoin/blob/master/src/script/interpreter.cpp#L98]]
    *
    * IMPORTANT: This function assumes signature has a SigHash byte and will
    * fail if not!
    *
    * @param signature
  ...
  /** This functions implements the strict der encoding rules that were created
    * in BIP66 https://github.com/bitcoin/bips/blob/master/bip-0066.mediawiki
    * [[https://github.com/bitcoin/bitcoin/blob/master/src/script/interpreter.cpp#L98]]
    *
    * IMPORTANT: This function assumes bytes has a SigHash byte and will fail if
    * not!
    *
    * @param bytes
  ...
  /** Scala implementation of
    ...
    *
    * "Supported violations include negative integers, excessive padding,
    * garbage at the end, and overly long length descriptors."
    *
    * The signatures must still follow the following general format:
    *
    * 0x30 | total length | 0x02 | R length | [R] \ 0x02 | S length | [S]
    *
    * IMPORTANT: Do not use this without further validation when validating
    * blocks because BIP 66 requires that new signatures are verified to be
    * strict DER encodings.
    */
