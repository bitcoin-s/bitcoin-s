...
/** Prior to the introduction of TypedArray in ECMAScript 2015 (ES6), the
  * JavaScript language had no mechanism for reading or manipulating streams of
  * binary data. The Buffer class was introduced as part of the Node.js API to
  * make it possible to interact with octet streams in the context of things
  ...
  * correspond to fixed-sized, raw memory allocations outside the V8 heap. The
  * size of the Buffer is established when it is created and cannot be resized.
  *
...
class Buffer() extends js.Object {
  ...
  /** Compares buf with target and returns a number indicating whether buf comes
    * before, after, or is the same as target in sort order. Comparison is based
    ...
    * @param targetEnd
    *   The offset with target at which to end comparison (not inclusive).
    *   Ignored when targetStart is undefined. Default: target.length
    * @param sourceStart
    *   The offset within buf at which to begin comparison. Ignored when
    *   targetStart is undefined. Default: 0
    * @param sourceEnd
    *   The offset within buf at which to end comparison (not inclusive).
    *   Ignored when targetStart is undefined. Default: buf.length
    * @return
    *   0 is returned if target is the same as buf 1 is returned if target
    *   should come before buf when sorted. -1 is returned if target should come
    *   after buf when sorted.
    * @example
  ...
  /** Copies data from a region of buf to a region in target even if the target
    ...
    * @param sourceStart
    *   The offset within buf at which to begin copying from. Ignored when
    *   targetStart is undefined. Default: 0
    * @param sourceEnd
    *   The offset within buf at which to stop copying (not inclusive). Ignored
    *   when sourceStart is undefined. Default: buf.length
    * @return
  ...
  /** Returns true if both buf and otherBuffer have exactly the same bytes,
    ...
    *   A Buffer to compare to
    * @return
    *   true if both buf and otherBuffer have exactly the same bytes, false
    *   otherwise.
    * @example
  ...
  /** Fills buf with the specified value. If the offset and end are not given,
    * the entire buf will be filled. This is meant to be a small simplification
    * to allow the creation and filling of a Buffer to be done on a single line.
    * @param value
  ...
  /** Returns the index of the first occurrence of value in buf or -1 if buf
    ...
    *   If value is a string, this is its encoding. Default: 'utf8'
    * @return
    *   The index of the first occurrence of value in buf or -1 if buf does not
    *   contain value
    * @example
  ...
  /** The largest size allowed for a single Buffer instance. On 32-bit
    * architectures, this value is (2^30)-1 (~1GB). On 64-bit architectures,
    * this value is (2^31)-1 (~2GB). Note that this is a property on the buffer
    * module returned by require('buffer'), not on the Buffer global or a Buffer
    * instance.
    * @return
  ...
  /** Identical to buf.indexOf(), except buf is searched from back to front
    ...
    *   If value is a string, this is its encoding. Default: 'utf8'
    * @return
    *   The index of the last occurrence of value in buf or -1 if buf does not
    *   contain value
    * @example
  ...
  /** Reads a signed 16-bit integer from buf at the specified offset with the
    * specified endian format (readInt16BE() returns big endian, readInt16LE()
  ...
  /** Reads a signed 32-bit integer from buf at the specified offset with the
    * specified endian format (readInt32BE() returns big endian, readInt32LE()
    * returns little endian).
    *
    * Setting noAssert to true allows offset to be beyond the end of buf, but
    * the result should be considered undefined behavior.
  ...
  /** Reads byteLength number of bytes from buf at the specified offset and
    ...
    * the result should be considered undefined behavior.
    * @param offset
    *   Where to start reading. Must satisfy: 0 <= offset <= buf.length -
    *   byteLength
    * @param byteLength
  ...
  /** Reads an unsigned 32-bit integer from buf at the specified offset with
    * specified endian format (readUInt32BE() returns big endian, readUInt32LE()
    * returns little endian). * Setting noAssert to true allows offset to be
    * beyond the end of buf, but the result should be considered undefined
    * behavior.
    * @param offset
    *   Where to start reading. Must satisfy: 0 <= offset <= buf.length - 4
    * @param noAssert
  ...
  /** Interprets buf as an array of unsigned 16-bit integers and swaps the
    * byte-order in-place. Throws a RangeError if buf.length is not a multiple
    * of 2.
    * @return
    *   A reference to buf
    * @example
  ...
  /** Interprets buf as an array of unsigned 64-bit numbers and swaps the
    * byte-order in-place. Throws a RangeError if buf.length is not a multiple
    * of 8.
    * @return
    *   A reference to buf
  ...
  /** Re-encodes the given Buffer instance from one character encoding to
    ...
    * specify invalid character encodings or if conversion from fromEnc to toEnc
    * is not permitted.
    * @param source
    *   A Buffer instance
    * @param fromEnc
  ...
  /** Writes string to buf at offset according to the character encoding in
    ...
    * not contain enough space to fit the entire string, only a partial amount
    * of string will be written. However, partially encoded characters will not
    * be written.
    * @param string
  ...
  /** Writes value to buf at the specified offset with specified endian format
    ...
    * value should be a valid 64-bit double. Behavior is undefined when value is
  ...
  /** Writes value to buf at the specified offset with specified endian format
    ...
    * value should be a valid 32-bit float. Behavior is undefined when value is
  ...
  /** Writes value to buf at the specified offset with specified endian format
    ...
    * value should be a valid signed 16-bit integer. Behavior is undefined when
  ...
  /** Writes value to buf at the specified offset with specified endian format
    ...
    * value should be a valid signed 32-bit integer. Behavior is undefined when
  ...
  /** Writes byteLength bytes of value to buf at the specified offset. Supports
    ...
    *   Number to be written to buf
    * @param offset
    *   Where to start writing. Must satisfy: 0 <= offset <= buf.length -
    *   byteLength
    * @param byteLength
  ...
  /** Writes value to buf at the specified offset with specified endian format
    ...
    * value should be a valid unsigned 16-bit integer. Behavior is undefined
  ...
  /** Writes value to buf at the specified offset with specified endian format
    ...
    * value should be a valid unsigned 32-bit integer. Behavior is undefined
  ...
  /** Writes byteLength bytes of value to buf at the specified offset. Supports
    * up to 48 bits of accuracy. Behavior is undefined when value is anything
    * other than an unsigned integer.
    *
    * Setting noAssert to true allows the encoded form of value to extend beyond
    * the end of buf, but the result should be considered undefined behavior.
    * @param value
...
object Buffer extends js.Object {
  ...
  /** On 32-bit architectures, this value is (2^30)-1 (~1GB). On 64-bit
    * architectures, this value is (2^31)-1 (~2GB).F Note that this is a
    * property on the buffer module returned by require('buffer'), not on the
  ...
  /** This is the number of bytes used to determine the size of pre-allocated,
    * internal Buffer instances used for pooling. This value may be modified.
    */
  ...
  /** Allocates a new non-zero-filled and non-pooled Buffer of size bytes. The
    ...
    * {{{(2^31)-1).}}} Otherwise, a RangeError is thrown. A zero-length Buffer
    * will be created if a size less than or equal to 0 is specified.
    *
    * The underlying memory for Buffer instances created in this way is not
    * initialized. The contents of the newly created Buffer are unknown and may
    * contain sensitive data. Use buf.fill(0) to initialize such Buffer
    * instances to zeroes.
    *
    * When using Buffer.allocUnsafe() to allocate new Buffer instances,
    * allocations under 4KB are, by default, sliced from a single pre-allocated
    * Buffer. This allows applications to avoid the garbage collection overhead
    * of creating many individually allocated Buffers. This approach improves
    * both performance and memory usage by eliminating the need to track and
    * cleanup as many Persistent objects.
    *
  ...
  /** Returns a new Buffer which is the result of concatenating all the Buffers
    * in the list together. If the list has no items, or if the totalLength is
    * 0, then a new zero-length Buffer is returned. If totalLength is not
    * provided, it is calculated from the Buffers in the list. This, however,
    * adds an additional loop to the function, so it is faster to provide the
    * length explicitly.
    * @param list
  ...
  /** Creates a new Buffer containing the given JavaScript string str. If
    * provided, the encoding parameter identifies the strings character
    * encoding.
    * @param str
    *   the source string
    * @param encoding
  ...
  /** Returns true if encoding contains a supported character encoding, or false
    ...
    * @return
    *   true if encoding contains a supported character encoding, or false
    *   otherwise.
    */
  def isEncoding(encoding: String): Boolean = js.native

  /** Re-encodes the given Buffer instance from one character encoding to
    * another. Returns a new Buffer instance.
    *
    * Throws if the fromEnc or toEnc specify invalid character encodings or if
    * conversion from fromEnc to toEnc is not permitted.
    *
    * The transcoding process will use substitution characters if a given byte
    * sequence cannot be adequately represented in the target encoding.
    * @param source
    *   A Buffer instance
    * @param fromEnc
    *   The current encoding
