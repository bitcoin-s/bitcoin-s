...
trait EclairTestUtil extends BitcoinSLogger {
  ...
  def randomEclairClient(bitcoindRpcOpt: Option[BitcoindRpcClient] = None)(implicit system: ActorSystem): EclairRpcClient = {
    val bitcoindRpc = {
  ...
  def cannonicalEclairClient()(implicit system: ActorSystem): EclairRpcClient = {
   ...
   */
  def createNodePair(bitcoindRpcClientOpt: Option[BitcoindRpcClient])(implicit system: ActorSystem): (EclairRpcClient, EclairRpcClient) = {
  ...
  def openChannel(
    ...
    val fundedChannelIdF: Future[FundedChannelId] = {
      n2.nodeId.flatMap { nodeId =>
        n1.getInfo.map { info =>
          logger.debug(s"Opening a channel from ${info.nodeId} -> ${nodeId} with amount ${amt}")
  ...
  def shutdown(eclairRpcClient: EclairRpcClient)(implicit system: ActorSystem): Unit = {
