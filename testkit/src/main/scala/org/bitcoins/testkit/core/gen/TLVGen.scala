...
trait TLVGen {
  ...
  def contractInfoV0TLV: Gen[ContractInfoV0TLV] = {
    for {
      ...
      val satVals = (0 until outcomes.length - 2).toVector.map { _ =>
        Satoshis(NumberUtil.randomLong(totalInput.toLong))
      } :+ totalInput :+ Satoshis.zero

      val outcomeValueMap = outcomes.toVector.zip(satVals)
      ContractInfoV0TLV(outcomeValueMap)
    }
  }
