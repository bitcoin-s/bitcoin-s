...
class ChainHandler(
    ...
    val blockFilterCheckpoints: Map[DoubleSha256DigestBE,
      ...
      for {
        ...
        fsmOptVec <- {
          Future.traverse(candidateStartHeaders) { candidateHeader =>
            getFilterSyncStopHash(candidateStartHeader = candidateHeader,
  ...
  private def getImmediateChildren(
      blockHashBE: DoubleSha256DigestBE): Future[Vector[BlockHeaderDb]] = {
  ...
  protected def getBestFilterHeaderWithChains(blockchains: Vector[Blockchain])
  ...
  override def getMedianTimePast(): Future[Long] = {
    ...
    @tailrec
    def getNTopHeaders(n: Int, acc: Vector[Future[Option[BlockHeaderDb]]])
