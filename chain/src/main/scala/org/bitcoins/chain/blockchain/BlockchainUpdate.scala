...
/** Represens the state of a batch of
  * [[org.bitcoins.core.protocol.blockchain.BlockHeader BlockHeaders]] being
  * added to our blockchain
  *
  * An example of a
  * [[org.bitcoins.chain.blockchain.BlockchainUpdate.Failed Failed]] update is
  * when we receive a
  * [[org.bitcoins.core.protocol.blockchain.BlockHeader BlockHeader]] that is
  * invalid and because of a
  * [[org.bitcoins.chain.validation.TipUpdateResult.Failure TipUpdateFailure]]
  * because of [[org.bitcoins.chain.validation.TipUpdateResult.BadPOW BadPOW]]
  * or a [[org.bitcoins.chain.validation.TipUpdateResult.BadNonce BadNonce]] etc
  */
sealed abstract class BlockchainUpdate {

  /** The successful headers in this batch blockchain update that need to be
    * persisted
    */
...
object BlockchainUpdate {
  ...
  /** The key thing we receive here is
    * [[org.bitcoins.chain.models.BlockHeaderDb BlockHeaderDb]] with a height
    * assigned to it this happens after calling
    * [[org.bitcoins.chain.blockchain.ChainHandler.processHeaders ChainHandler.processHeaders]]
    */
  ...
  /** Means we failed to update the given blockchain with _ALL_ given headers
    * This means we could have had a partially successful update, with the
    * headers/blockchain returned in this case class
    */
  case class Failed(
      blockchain: Blockchain,
      successfulHeaders: Vector[BlockHeaderDb],
